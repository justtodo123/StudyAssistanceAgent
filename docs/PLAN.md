...
| M10 | `BLOCKED` | `NOT_STARTED` | [`m10-autonomous-runner-plan.md`](plans/m10-autonomous-runner-plan.md) | 三项前置 `M10-M7-EXIT` / `M10-M8-EXIT` / `M10-M9-EXIT` 自 2026-09-22 起**均已 `SATISFIED`**——其中 `M10-M8-EXIT` 以 owner 当日批准的**维持现状结论**兑现（数据面维持 SQLite/BM25，100K 专业化存储容量验证保持待命参考，见 M10 计划 §2.1），**不是容量证据**；阻断项只剩十一项强制决策与 owner 的独立准入批准。**十一项中两项已于 2026-09-22 由 owner 批准 `RESOLVED`**（闭合批次 ①：`M10-AUTHORITY`、`M10-WRITE-AUTHORIZATION`，见 [`m10-decision-closure-v1.md`](plans/references/m10-decision-closure-v1.md)），其余九项（checkpoint、幂等、EffectLedger、恢复、评测、manifest、MCP、offline-default、rollout）仍 `OPEN`，分批次 ②③ 推进 |
