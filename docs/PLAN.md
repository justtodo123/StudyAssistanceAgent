...
> M7 退出前置已满足，但 M8–M12 自身仍为 `BLOCKED / NOT_STARTED`；M8 active execution protocol `draft-0.10`
...
| M8 | `BLOCKED` | `NOT_STARTED` | [计划](plans/m8-specialized-storage-plan.md) | 八项 Decision 已 `RESOLVED`；`draft-0.10` 曾到达 `BINDING_FROZEN`，但独立 P3 因三项阻断缺陷 `REJECTED / stop`；不得 `request-p4`，执行、后端选择和阶段 admission 均未授权 |
...
*创建：2026-08-10 · PLAN 文档修订：v2.23（不是产品发布版本）· 更新：2026-09-14（M8 八项 Decision 已
`RESOLVED`；active execution protocol `draft-0.10` 曾到达 `BINDING_FROZEN`，但独立 P3 因三项阻断缺陷判定
`REJECTED / stop`；不得 `request-p4`，M8 仍为 `BLOCKED / NOT_STARTED`，从未执行，后端选择与阶段批准均未完成）·
维护：每次会话开工查看本文档*
