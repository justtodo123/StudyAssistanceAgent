...
M8 与 M11–M12 仍为 `BLOCKED / NOT_STARTED`。M8 的八项 Decision 已于 2026-09-10 逐项批准并 `RESOLVED`，但这只关闭
...
数值门槛和报告字段，历史 V1–V13 数值或方法不能替代该冻结。M9 已于 2026-09-22 在 `m9-plan-lifecycle-v1` 范围内
取得独立完成批准，现为 `ADMITTED / COMPLETE`；**M10 的十一项 Decision 已全部 `RESOLVED`，并于 2026-09-22 在
`m10-autonomous-runner-v1` 范围内获批为 `ADMITTED / IN_PROGRESS`、`implementation_start` 同步 `AUTHORIZED`**
（三项前置均 `SATISFIED`，其中 `M10-M8-EXIT` 以维持现状结论兑现）；M11/M12 是新登记的拟议阶段，全部 Decision、
前置和批准保持开放。M6a-P0 crawler 本身只构成
前置证据，不单独批准阶段。
