...
> 文档状态：`DRAFT / P0_REVIEW_OPEN / UNBOUND / NOT_STATICALLY_AUDITED / NOT_AUTHORIZED / NEVER_EXECUTED`
> 文档版本：`draft-0.2`（仅为文档修订号，不是 experiment ID 或 executable protocol ID）
> 修订依据：[`draft-0.1` P0 前技术审查](m8-active-execution-protocol-draft-0.1-review-20260911.md)退回；本版修复 query 分母、证据发布、launcher/TEMP 和 exact/flat 边界
> 起草日期：2026-09-11
...
本协议只回答一个问题：在现行 SQLite/M7 控制面和默认离线路径完全不变时，LanceDB 嵌入式 **exact/flat** 数据面
能否在固定的 1K/10K/100K synthetic workload 上满足正确性、生命周期、资源和本地单用户收益门槛，从而成为后续
人工选择 exact/flat profile 的合格输入。本轮不评价 ANN、IVF、HNSW、PQ 或任何近似索引；即使 P9 选择 LanceDB，
批准范围也仅限本协议验证的 exact/flat profile。未来引入 ANN 必须使用全新协议与身份，冻结索引参数和 recall 门槛。
...
| 顺序 | 决定 | 当前状态 | 允许的下一步 |
| --- | --- | --- | --- |
| `P0` | 接受本文 `draft-0.2` 技术口径 | `OPEN` | `draft-0.1` 已退回；本版仅允许记录接受或再次退回 |
...
| `P7` | 接受独立结果核验 | `OPEN` | 只确认 raw evidence disposition，不改变阶段状态 |
| `P7A` | 允许发布脱敏 evidence package | `OPEN` | 只写负责人批准的 durable archive；不得改变阶段或后端状态 |
| `P8` | 是否准入 M8 | `OPEN` | 必须引用 durable evidence package 并另行更新 PLAN 与 registry |
| `P9` | 是否选择某个后端 | `OPEN` | 与 P8 分开；可选择“不采用”，必须引用 durable evidence package |

P0-P5 的任何活动都不是 benchmark execution。P6 必须引用 exact experiment/protocol identity、binding、候选版本、
允许层级、临时根、时限和 cleanup 责任。P7 验证 raw evidence 后，P7A 必须另行冻结 durable archive 的目标路径、
allowlist、schema、writer、canonical bytes、digest 和 O_EXCL 写入范围；未取得 P7A 时只能 cleanup 并形成
`VALID_EVIDENCE_NOT_PUBLISHED`，不得进入 P8/P9。P7-P9 禁止由脚本、分数、平均值或本草案自动生成。
...
| active candidate | `lancedb-embedded-exact` | 本地 embedded、exact/flat、无服务；不含 ANN | 仅 P9 可决定，且批准范围只能是 exact/flat |
...
| experiment parent | P2/P4 时冻结的专用本地 NTFS 绝对父目录；不依赖继承 `%TEMP%`/`%TMP%` |
...
| process topology | 单个 benchmark coordinator；每个 query repetition 独立 child process；每个 build/lifecycle sample 独立 child process |
...
起草时观察到系统默认 `python` 是 `3.13.3`，不是本协议解释器；进程继承环境还会使冻结 launcher 误加载 WPS 灵犀
Python 标准库并触发 `SRE module mismatch`。因此 executable SHA-256 不能单独构成 launcher health。P4 前必须以隔离
child environment 验证：删除 `PYTHONHOME`/`PYTHONPATH` 及非授权 Python 路径，显式使用冻结 launcher，启用 isolated
mode（至少 `-I`），并机械确认 `sys.version_info`、`sys.executable`、`sys.prefix/base_prefix`、`sys.path` 和标准库来源；
`re/json/hashlib/sqlite3/venv` import 必须成功且不得解析到 WPS 灵犀或系统 3.13 环境。任一失败为
`ABORTED_PREFLIGHT`，且发生在 candidate import、venv/source/corpus 创建之前。

起草时观察到继承 `%TEMP%=D:\tmp`，与旧草案的 C: 假设冲突；本版不再依赖继承 TEMP。P2/P4 必须单独冻结专用
experiment parent 的绝对路径、volume serial、filesystem、file ID、free bytes 和权限，并把 child `TEMP`/`TMP`、pip
cache、Arrow/Lance 临时目录重定向到 experiment root 内。起草时可用物理内存不足 8 GiB，因此当前主机不满足下述
执行前置，本次不得执行。
...
- P2/P4 冻结的 experiment parent 所在卷 available free space `>= 16 GiB`；
...
- experiment parent 的 path/volume/filesystem/file ID 与 binding 一致；唯一临时根解析后位于该 parent，且此前不存在；
- child `TEMP`/`TMP`、pip cache、Arrow/Lance temp 均解析到实验根内，继承的外部 temp/cache 路径不可见。
...
这些版本是规范性草案值，不是对历史运行结果的引用。P4 必须进一步拆为“限时 acquisition”与“断网 preparation”两个
互斥子阶段：acquisition 授权精确冻结 index URL、允许的 HTTPS host/redirect、TLS 证书验证、proxy policy、timeout、
wheel-only、完整直接/传递依赖 allowlist、文件名、版本、size、SHA-256、license 和下载目标；仅允许 Windows x86-64 /
CPython 3.11 wheel，禁止 sdist、VCS、editable、`latest`、范围版本和运行期解析。下载结束后先验证 canonical lock digest，
再物理断网进入 preparation/benchmark；benchmark 阶段不得访问 index、cache 或网络。任一 wheel/metadata 不可得、重定向
越界或不兼容时协议退回修订，不得替换版本后继续同一身份。
...
candidate 不参与 gold 生成。gold 只在 input-freeze 阶段由独立 generator 计算一次，执行阶段读取已冻结 gold，不得把
oracle exhaustive 计算时间计入 candidate query latency。P4/P5 必须分别记录 gold generation wall-clock、CPU、peak RSS、
disk、generator source digest 和 output digest；超过当时冻结的生成预算即停止，不得分批拼接未审结果。

| workload | measured hit | measured exact filtered hit | measured deny/no-hit | measured unique | warmup-only | manifest total |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `1k-correctness` | 100 | 50 | 50 | 200 | 20 | 220 |
| `10k-single-user` | 300 | 100 | 100 | 500 | 50 | 550 |
| `100k-capacity` | 800 | 0 | 200 | 1,000 | 100 | 1,100 |
| `100k-filtered` | 400 | 400 | 200 | 1,000 | 100 | 1,100 |
...
warmup-only query 使用独立 `query_kind=warmup` 从未进入 measured 集的 active rows 选择，query ID 固定
`warmup-{workload}-{ordinal:04d}`；与 measured query ID、target chunk 和 gold entry 均不得重叠。warmup 只验证同一路径
和缓存预热，不进入 percentile、Recall、MRR 或 no-hit 分母。

每个 repetition 复用同一冻结的 measured unique query 集，只由
`SHA256(namespace || workload || repetition_uint32_le || query_id)` 决定执行顺序，不 replacement、不循环、不新增 query。
固定 `top_k` 分布为 1/3/5，各占 measured hit queries 的 `20%/40%/40%`；no-hit 固定 `top_k=5`。所有 warmup、measured
query、filter、gold、score、排序、集合角色与分母必须写入 canonical query/gold manifest，并在 candidate import 前冻结
单一 SHA-256。
...
每个 backend/workload/repetition 先执行该 workload 的全部 warmup-only query，再执行全部 measured unique query；
两者均使用冻结 query ID，backend 间顺序完全一致。query sample 指单次 query observation；query repetition 指一个独立
child process，内含该层全部 warmup 与 measured query。build sample 和 lifecycle sample 各自指一个独立 child process
及独立 sample root。warmup 不进入统计但必须单独计数；build 每个 measured sample 使用新空目录，禁止累积。
...
- `canonical input bytes` 精确定义为该 workload canonical corpus JSONL（包含固定 metadata 与 little-endian float32 vector
  bytes 的 base64 字段）在未压缩状态下的实际 byte length，不含 SQLite oracle、query/gold、source、venv、wheel、log、
  report 或 sample root；报告必须同时记录 raw vector bytes、metadata bytes、JSONL bytes 和各自 SHA-256；
...
P4 获批后，唯一实验根必须是 P2/P4 已冻结 experiment parent 下当时不存在的子目录，名称由新 experiment ID 的
SHA-256 前 16 位和 128-bit CSPRNG nonce 组成。该 parent 不从继承 `%TEMP%`/`%TMP%` 推导。所有 venv、wheel、source、
corpus、gold、index、sample、raw report、日志及 child temp/cache 只能位于该根。
...
P0 protocol accepted
  ...
  -> L4 100K filtered
  -> execution report
  -> P7 independent raw-evidence verification
  -> P7A sanitized durable-publication authorization
  -> write-once sanitized evidence package outside experiment root
  -> cleanup and residual verification
  -> finalize durable evidence disposition and package digest
...
| `VALID_EVIDENCE_NOT_PUBLISHED` | raw evidence 可核验但未取得 P7A durable publication 授权 | 否 |
...
独立核验者先在只读状态下核对 raw evidence，并生成尚未发布的脱敏 evidence package canonical bytes。只有 P7A 明确
授权后，核验者才可用已审计 write-once writer 将 package 写入实验根之外的冻结目标；目标必须是 repository-relative
治理凭证路径或负责人批准的绝对 archive 路径，二者只能选一，写前必须不存在。P7A 必须绑定目标、exact file allowlist、
package schema、canonical SHA-256、writer source digest、O_CREAT|O_EXCL、fsync 和禁止字段清单。

脱敏 durable package 至少包含 verification report、execution report digest、input/source/harness/dependency digests、完整
sample/gate inventory、violations、cleanup plan、批准引用和预期最终 disposition；禁止正文、raw vectors、credential、
完整环境变量、用户绝对路径和 backend 原始异常。P7A 未授权或 durable 写入失败时，合法终态只能是
`VALID_EVIDENCE_NOT_PUBLISHED` 或 `INVALID_PROTOCOL_DEVIATION`，不得进入 P8/P9。

package 持久化并独立回读 digest 后，核验者触发已审计 cleanup。cleanup 必须关闭 backend、释放 handle、终止
allowlisted child、删除全部 experiment-root 内 sample/index/corpus/wheel/venv/source/log/report 临时对象，最后删除
experiment root 本身；不得删除或修改 durable package。核验者确认 root 不存在、父目录无同 nonce sibling、residual
bytes/entries/handles/process/listeners 为零后，以 write-once companion receipt 或 package 中预留的 detached cleanup receipt
记录最终 cleanup digest。cleanup 失败永远覆盖性能结论，最终 disposition 固定 `CLEANUP_INCOMPLETE`，并要求 durable
package/companion receipt 如实记录失败；不得覆写先前文件。
...
| 决定 | 负责人 | 日期 | 书面引用 | 结论 |
| --- | --- | --- | --- | --- |
| P0 接受 `draft-0.2` 协议文本 | - | - | `draft-0.1 RETURNED_FOR_REVISION` | `OPEN` |
| P1 允许创建新实验身份 | - | - | - | `OPEN` |
...
| P6 允许受控执行 | - | - | - | `OPEN` |
| P7 接受独立结果核验 | - | - | - | `OPEN` |
| P7A 允许发布脱敏 evidence package | - | - | - | `OPEN` |
| P8 M8 admission | - | - | - | `OPEN` |
| P9 backend selection | - | - | - | `OPEN` |

## 13. 当前结论

本文完成后仍只是 `DRAFT / P0_REVIEW_OPEN / UNBOUND / NOT_STATICALLY_AUDITED / NOT_AUTHORIZED / NEVER_EXECUTED`。它不创建新的实验
