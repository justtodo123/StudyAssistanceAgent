...
> 文档版本：`draft-0.6`（仅为文档修订号，不是 experiment ID 或 executable protocol ID）。
> 起草日期：2026-09-12，修订日期：2026-09-13。本文为 draft-0.6 修订；本文所有摘要均由外部记录保存，本文不自哈希。
> 修订依据：draft-0.5 的 stream 约束假定 volume root、directory 与 file 只暴露 unnamed stream，但 Windows 的
> `:sguard:$DATA` 系统保留流会出现在卷根与目录上，使具备该 OS 安全特性的机器上不存在任何可通过 P2 的卷。
> draft-0.6 将系统保留流从用户 ADS 中分离，并以显式、闭合、可机械复验的 allowlist 处理，见 §2.2、§7 与
> `sa.m8.child-allowlist.v1.payload` 的 `allowed_system_streams`。draft-0.5 保持历史，不得 binding 或授权。
> 协议字节的任何变化都会改变 `protocol_blob_sha256`，所有绑定 draft-0.5 摘要的 P0/P1 记录均因此失效，
> 必须在新摘要上重新形成 P0、P1 与 P2。
...
`SYSTEM_RESERVED_STREAM` 是闭合枚举，当前唯一成员为 `sguard`，序列化形式为 string `"sguard"`，对应 Windows 上的
`:sguard:$DATA`。该枚举只描述由操作系统组件在卷、目录或文件上维护、且实验进程无权创建、写入或删除的数据流。
它的每个成员都必须按 `leaf_kind` 显式列入 `sa.m8.child-allowlist.v1.payload.allowed_system_streams` 才被允许；
...
- `sa.m8.child-allowlist.v1.payload`：
  ...
  allowed_system_streams:A<{leaf_kind:enum[file,directory],stream:SYSTEM_RESERVED_STREAM};0..2;
  order=key(leaf_kind,stream);unique=key(leaf_kind,stream)>,
...
Rows 1–12 are the complete file/process symbol coverage, with semantic operation groups frozen by the operation matrix: `parent-walk` and `post-delete-reopen` use their distinct `NtCreateFile` wrappers; `query-file-metadata`, `query-streams` and `enumerate-parent` use `query-file-information-v1`; `rename-no-replace` and `delete-disposition` use `file-mutation-v1`. Rows 13–15 are independent network symbols; `GetExtendedTcpTable` covers listener inventory, while `WSAConnect` and `connect` each cover outbound denial. Both listener detection and outbound denial use the same phase-qualified `STABLE_CODE`, selected by actual phase as `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` or `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED`. There is no separate listener-detection code: a listener row is an outbound-containment failure and carries the identical phase-qualified value. Any raw `OUTBOUND_BLOCKED`, `CONTAINMENT_UNSUPPORTED`, or any `RELISTENER_*`/`LISTENER_DETECTED` alias is invalid. Any module/symbol/wrapper/deny_code/pre-open/post-check mismatch yields the applicable phase-qualified `STABLE_CODE` and `ABORTED_PREFLIGHT` before candidate import or `INVALID_PROTOCOL_DEVIATION` during execution.
  ...
  are forbidden; absent coverage for any one operation yields `PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED` and `ABORTED_PREFLIGHT`.
...
`NumberOfLinks=1`。stream 规则按 `leaf_kind` 与 `FileStandardInformation.Directory` 二分，并且是闭合的：
`file` leaf 必须恰有 unnamed stream `:$DATA`，且除该 unnamed stream 外只允许出现 allowlist 中该 `leaf_kind` 冻结的
`SYSTEM_RESERVED_STREAM`；`directory` 与 volume/device root handle 允许 unnamed stream 不出现，且除 allowlist 中该
`leaf_kind` 冻结的 `SYSTEM_RESERVED_STREAM` 外不得有任何 stream 记录。任何未列入 allowlist 的 named stream、任何
缺失的 unnamed stream、任何 allowlist 外部观测到的 stream 记录，均使本次操作 fail closed 并映射为稳定 CODE；
执行前为 `ABORTED_PREFLIGHT`，执行中为 `INVALID_PROTOCOL_DEVIATION`，cleanup 中为 `CLEANUP_INCOMPLETE`。
禁止 symlink、junction、mount point、任何其他 reparse、用户 ADS、hard link、跨卷、case-alias collision 和 identity swap。rename 使用
