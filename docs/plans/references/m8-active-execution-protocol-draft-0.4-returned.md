...
> 文档版本：`draft-0.4`（仅为文档修订号，不是 experiment ID 或 executable protocol ID）。
> 起草日期：2026-09-12。本文为 draft-0.4 修订；本文所有摘要均由外部记录保存，本文不自哈希。
...
Rows 1–12 are the complete file/process symbol coverage, with semantic operation groups frozen by the operation matrix: `parent-walk` and `post-delete-reopen` use their distinct `NtCreateFile` wrappers; `query-file-metadata`, `query-streams` and `enumerate-parent` use `query-file-information-v1`; `rename-no-replace` and `delete-disposition` use `file-mutation-v1`. Rows 13–15 are independent network symbols; `GetExtendedTcpTable` covers listener inventory, while `WSAConnect` and `connect` each cover outbound denial. Their runtime denial code is selected by actual phase as `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` or `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED`; listener detection uses `RELISTENER_PREFLIGHT_LISTENER_DETECTED` or `RELISTENER_RUNTIME_LISTENER_DETECTED` in the corresponding network evidence row. Any raw `OUTBOUND_BLOCKED` or `CONTAINMENT_UNSUPPORTED` alias is invalid. Any module/symbol/wrapper/deny_code/pre-open/post-check mismatch yields the applicable phase-qualified `STABLE_CODE` and `ABORTED_PREFLIGHT` before candidate import or `INVALID_PROTOCOL_DEVIATION` during execution.
  ...
  two process rows, and one network row. The network row's three API alternatives are independently covered by interposition rows 13–15; `GetExtendedTcpTable` supplies listener inventory and `WSAConnect` plus `connect` supply outbound denial. Any `WSAConnect` or `connect` call is intercepted before network access and maps to the phase-qualified `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` or `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED`; listener detection maps to the corresponding `RELISTENER_PREFLIGHT_LISTENER_DETECTED` or `RELISTENER_RUNTIME_LISTENER_DETECTED`. Any missing discriminator field, mismatched matrix value, or unlisted operation fails closed.
...
Cross-field combinations are closed：`branch=abort` 只允许 trigger `{ABORTED_PREFLIGHT,ABORTED_RUNTIME,INVALID_PROTOCOL_DEVIATION,REJECTED_INPUT_AUDIT,REJECTED_RAW_EVIDENCE,VERIFICATION_TIMEOUT,REPORT_TIMEOUT}`，且 `pending_terminal` 必须逐字等于 `trigger` 经下列唯一映射得到的 abort terminal：`ABORTED_PREFLIGHT→ABORTED_PREFLIGHT`、`ABORTED_RUNTIME→ABORTED_RUNTIME`、`INVALID_PROTOCOL_DEVIATION→INVALID_PROTOCOL_DEVIATION`、`REJECTED_INPUT_AUDIT→REJECTED_INPUT_AUDIT`、`VERIFICATION_TIMEOUT→REJECTED_RAW_EVIDENCE`、`REPORT_TIMEOUT→ABORTED_RUNTIME`、`REJECTED_RAW_EVIDENCE→REJECTED_RAW_EVIDENCE`；任何其他组合均为 schema failure。authorization refs 在 P6 前恰为 `[P4]`、P6 后恰为 `[P4,P6]`。
`branch=normal` 只有在该失败确实终止于 `CLEANUP_INCOMPLETE` 时合法，只允许 trigger `{CLEANUP_TIMEOUT,RECEIPT_CREATE_FAILED,RECEIPT_WRITE_FAILED,RECEIPT_READBACK_FAILED,RECEIPT_VALIDATION_FAILED}`，`pending_terminal` 允许 `ABORTED_PREFLIGHT`、`ABORTED_RUNTIME`、`INVALID_PROTOCOL_DEVIATION` 三个值且必须逐字等于触发该 cleanup 的层 FAIL 所产生的 abort terminal（与 abort receipt 的同一字段逐字一致），authorization refs 恰为 `[P6,P7A]`。`branch=nonpublication` 只允许 trigger `{P7A_REFUSED,PACKAGE_TIMEOUT,PACKAGE_CREATE_FAILED,PACKAGE_WRITE_FAILED,PACKAGE_READBACK_FAILED,PACKAGE_VALIDATION_FAILED,CLEANUP_TIMEOUT,RECEIPT_CREATE_FAILED,RECEIPT_WRITE_FAILED,RECEIPT_READBACK_FAILED,RECEIPT_VALIDATION_FAILED}`，`pending_terminal` 为上述三个值之一；若 `trigger=P7A_REFUSED` 则 authorization refs 恰为 `[P6]`，若是 package failure（含 `PACKAGE_TIMEOUT`）则恰为 `[P6,P7A]`，若是 cleanup/receipt failure 则 refs 必须逐字沿用被替代 nonpublication receipt 本应使用的 `[P6]` 或 `[P6,P7A]`。任何未列 branch/trigger/pending-terminal/ref tuple 都是 schema failure。`pending_terminal` 在该 record 中只作为已冻结的 abort terminal 标签携带，绝不表示已完成 abort cleanup，且 P8/P9、`ABORTED_CLEANUP_VERIFIED`、`VALID_EVIDENCE_NOT_PUBLISHED` 永不出现。attempted receipt 和 failure-record target 必须分别逐字等于适用 P4/P7A target，且互异。`attempted_receipt_writer_result_ref` 必须解析到 attempted receipt target 的 failure variant，`failure_record_writer_result_ref` 必须解析到 failure-record target 的 success variant；任一 REF 缺失、未解析、variant 不匹配或与 target 不符均为 schema failure。两个 writer result 都是 out-of-band attestation：它们逐字绑定各自 target、canonical bytes、CREATE_NEW 结果和 readback，不得由被证明 bytes 引用自身。
...
raw_status:CODE,stable_code:STABLE_CODE}`。`entries` 是以下 72 行的精确有序集合，不是名称集合；序列化时按
