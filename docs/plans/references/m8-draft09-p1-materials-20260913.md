...
1. **`forbidden_history_ids` 标注为 `ID`，但取值含点号，与 `ID` 不匹配**
   ...
   该矛盾是确定的（值确实不匹配 `ID`，也确实匹配 `SCHEMA_ID` 的字符类且未违反其额外限制），
   但**修法并不唯一**：`SCHEMA_ID` 的字符类恰是 `ID` 的超集（仅多允许点号），所以两种修法都能消除矛盾——
   （a）若本意是允许点号，则标注改 `SCHEMA_ID`；（b）若本意是不允许点号，则应改那 13 个值而非改类型。
   本记录只证明**矛盾存在**，不判定哪种修法正确；选哪种取决于协议作者的本意。
...
2. **`order=value` 的排序结果与实际记录冲突（定义本身并不含混）**
   ...
   **不存在“语义未明确”**。但按此定义，这 13 个值应排为 `v1, v10, v11, v12, v13, v2, …, v9`（字典序）。
   ...
   与第 1 项同理，本记录只证明**不一致存在**，不判定哪一侧应改。
