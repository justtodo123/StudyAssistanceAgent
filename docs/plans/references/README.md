...
| [`m10-decision-closure-v1.md`](m10-decision-closure-v1.md) | M10 决策闭合记录 v1（**提案**） | 非授权；本文件**不产生** `RESOLVED`。分批推进（owner 2026-09-22 裁定）：**批次 ① 已获批准**——`M10-AUTHORITY` 与 `M10-WRITE-AUTHORIZATION` 在登记表中已转 `RESOLVED`（批准引用逐字为「按顺序进行即可」）；**批次 ②③ 已起草但未批准**，对应九项在登记表中仍 `OPEN`、`value` 为 `null`。§1/§2/§3 各含可直接落登记表的 `value` 令牌串与十项必备内容 |
