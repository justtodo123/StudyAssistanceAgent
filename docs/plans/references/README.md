...
| [`m10-decision-closure-v1.md`](m10-decision-closure-v1.md) | M10 决策闭合记录 v1（**提案**） | 非授权；十一项 Decision 仍 `OPEN`，本文件**不产生** `RESOLVED`。分批推进（owner 2026-09-22 裁定）：批次 ① `AUTHORITY` + `WRITE-AUTHORIZATION` 已起草（含可直接落登记表的 `value` 令牌串与十项必备内容），批次 ②③ 待写。§批准记录为空，批准前不得改登记表 |
