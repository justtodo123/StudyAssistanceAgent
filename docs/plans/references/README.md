...
| [`m10-decision-design-draft.md`](m10-decision-design-draft.md) | M10 十一项决策设计草案 | 非授权设计输入；十一项 Decision 仍 `OPEN`；不产生 `RESOLVED`、准入或开工。§0.1 逐条登记起草时核实的仓库事实（含 `learning_store.py` **无迁移机制**、job/EffectLedger/outbox/补偿/crash matrix **在代码中完全不存在**、`TOOL_PERMISSION_DENIED`/`BUDGET_EXCEEDED` 为**预留但未被消费**的契约码）；§末列四项**待 owner 裁定**的开放点，草案不自行决定 |
