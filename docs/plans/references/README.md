...
| `m8-network-acquisition-cycle-independent-review-request-20260919-r05.md` | Historical additive r05 independent scope-review request | Superseded by r06 transition-wording correction; never grants acquisition authority |
| `m8-network-acquisition-cycle-independent-review-prompt-20260919-r05.md` | Historical additive r05 independent scope-review prompt | Superseded by r06 transition-wording correction; not dispatched and not authority-granting |
...
| [`m8-network-acquisition-authority-remediation-review-request-20260919-r03.md`](m8-network-acquisition-authority-remediation-review-request-20260919-r03.md) | Remediation r03 independent blocked-candidate review request | Binds the frozen review target and exact staged facts; read-only and non-authorizing |
| [`m8-network-acquisition-authority-remediation-review-prompt-20260919-r03.md`](m8-network-acquisition-authority-remediation-review-prompt-20260919-r03.md) | Remediation r03 independent blocked-candidate review prompt | Restricts review to offline evidence verification; no acquisition or execution authority |
| [`external-artifacts/m8-network-acquisition-authority-remediation-review-dispatch-20260919-r03.json`](external-artifacts/m8-network-acquisition-authority-remediation-review-dispatch-20260919-r03.json) | Remediation r03 final blocked-candidate review dispatch manifest | Canonical final binding for read-only review; only permitted conclusion is blocked; non-authorizing |
| [`m8-network-acquisition-authority-remediation-downloader-prior-validator-20260919-r01.md`](m8-network-acquisition-authority-remediation-downloader-prior-validator-20260919-r01.md) | Additive Downloader Prior Validator report | Exact downloader object and Owner-supplied frozen specification absent; `DOWNLOADER_PRIOR_VALIDATION_REJECTED_FAILED_CLOSED`; no authority |
| [`m8-network-acquisition-authority-remediation-collector-authority-review-20260919-r01.md`](m8-network-acquisition-authority-remediation-collector-authority-review-20260919-r01.md) | Additive independent Collector Authority Review | Full S1 observer and separate real collector authority not frozen; `COLLECTOR_AUTHORITY_REVIEW_REJECTED_FAILED_CLOSED`; no collector or receipts |
...
| [`m9-decision-design-draft.md`](m9-decision-design-draft.md) | M9 八项决策设计草案 | 非授权设计输入；八项 Decision 仍 `OPEN`；Planner 只提建议、确定性优先、输入有界；不产生 `RESOLVED`、准入或开工。§7.1 为 v1.5 延迟/成本冻结的口径记录（**仅描述**已在登记表生效的值，本身仍不授权） |
| [`m10-completion-evidence-v1.md`](m10-completion-evidence-v1.md) | M10 完成证据汇总 v1 | 汇总 `m10-autonomous-runner-v1` 七项技术退出条件、实现/测试证据与 11 条已知限制；文件本身不产生批准，owner 已于 2026-09-23 另行批准 `M10 COMPLETE`，限制随收口接受而未解除 |
| [`m10-test-plan-v1.md`](m10-test-plan-v1.md) | M10 写副作用测试方案 v1 | **准入准备物**；关闭 M10 计划 §4 的「写副作用 crash-point、越权和重放测试方案可执行」一项。给出 10 点 crash matrix 的注入方式与断言、越权矩阵、重放/冲突、台账与 reconcile、权威边界与零写入的机械证明形态、恒等操作与隔离，以及**冻结的 Agent 评测任务集形状**（冻结的是任务类别 × 期望终态 × 越权探针，**不**冻结具体语料——Runner 尚不存在）。**不**创建 `tests/M10/`、**不**批准准入、**不**产生性能声明 |
| [`m10-decision-closure-v1.md`](m10-decision-closure-v1.md) | M10 决策闭合记录 v1 | 分批推进（owner 2026-09-22 裁定）。**批次 ①②③ 均已获批准**，十一项 Decision 在登记表中**全部 `RESOLVED`**（批准引用逐字为「按顺序进行即可」与「全部批准（推荐）」）。§1/§2/§3 各含可直接落登记表的 `value` 令牌串与十项必备内容。**注意**：决策闭合**不等于** M10 准入——`admission_status` 仍 `BLOCKED`、`approval` 五字段仍为空 |
| [`m10-decision-design-draft.md`](m10-decision-design-draft.md) | M10 十一项决策设计草案 | 非授权设计输入；十一项 Decision 仍 `OPEN`；不产生 `RESOLVED`、准入或开工。§0.1 逐条登记起草时核实的仓库事实（含 `learning_store.py` **无迁移机制**、job/EffectLedger/outbox/补偿/crash matrix **在代码中完全不存在**、`TOOL_PERMISSION_DENIED`/`BUDGET_EXCEEDED` 为**预留但未被消费**的契约码）；§末记录 2026-09-22 owner 对四项开放点的**裁定**（存储机制取独立 SQLite 文件、预留错误码由 M10 消费、维持单 worker、评测取 0 容忍组）——裁定只关闭草案内开放点，**不产生任何 `RESOLVED`** |
...
| `m8-minimal-1k-v3-s1-network-acquisition-remediation-handoff-20260918-r05.md` | v3 S1 network-acquisition Builder remediation r05 | 历史记录名；当前 checkout 未包含该文档；additive、未独立审查，不授权 acquisition 或 M8 执行 |
| `m8-minimal-1k-v3-s1-network-acquisition-builder-self-check-20260918-r01.md` | v3 S1 network-acquisition Builder 对抗自查 r01 | 历史记录名；当前 checkout 未包含该文档；`FAILED_CLOSED`，不是独立 Reviewer 结论 |
| `m8-minimal-1k-v3-s1-network-acquisition-builder-self-check-20260919-r02.md` | v3 S1 network-acquisition Builder 对抗自查 r02 | 历史记录名；当前 checkout 未包含该文档；`FAILED_CLOSED`，不是独立 Reviewer 结论 |
