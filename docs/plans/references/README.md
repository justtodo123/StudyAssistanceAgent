...
| [`m10-decision-closure-v1.md`](m10-decision-closure-v1.md) | M10 决策闭合记录 v1（**提案**） | 分批推进（owner 2026-09-22 裁定）。**批次 ①②③ 均已获批准**，十一项 Decision 在登记表中**全部 `RESOLVED`**（批准引用逐字为「按顺序进行即可」与「全部批准（推荐）」）。§1/§2/§3 各含可直接落登记表的 `value` 令牌串与十项必备内容。**注意**：决策闭合**不等于** M10 准入——`admission_status` 仍 `BLOCKED`、`approval` 五字段仍为空 |
