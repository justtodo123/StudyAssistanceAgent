...
> 文档版本：`draft-0.8`（仅为文档修订号，不是 experiment ID 或 executable protocol ID）。
> 起草日期：2026-09-12，修订日期：2026-09-13。本文为 draft-0.8 修订；本文所有摘要均由外部记录保存，本文不自哈希。
...
> stream 查询信息类的遗漏。
> 修订依据（三）：draft-0.7 的 P0 技术文字审查再次判定 `RETURNED_FOR_REVISION / P0_NOT_ACCEPTED`，draft-0.8 修正其三项
...
> draft-0.5/draft-0.6/draft-0.7 保持历史，不得 binding 或授权。协议字节的任何变化都会改变 `protocol_blob_sha256`，
...
- `STREAM_SCOPE`：`enum[file,directory,volume-root]`，是一个三值闭合枚举，不是 `leaf_kind` 的别名。
  ...
  `STREAM_SCOPE` 只由该次操作在 `NT_OPEN_PROFILE` 中冻结的 `opens_volume_root` 与 `leaf_kind` 两个字段决定，不使用
  `authoritative_handle_source`、不依赖运行时推断、不做路径比较，因此对每个 profile 都是常量：
  `opens_volume_root=true` 时取 `volume-root`；否则 `leaf_kind=file` 取 `file`，`leaf_kind=directory` 取 `directory`。
  `opens_volume_root=true` 只允许出现在 `leaf_kind=directory` 的 profile 上，违反即 `ABORTED_PREFLIGHT`。
  三者互斥且穷尽：任一 `(opens_volume_root,leaf_kind)` 组合都有且只有一个取值，因此不存在无法选定 scope 的 handle；
  出现未列出的组合即为 `ABORTED_PREFLIGHT`。
...
- `sa.m8.child-allowlist.v1.payload`：
  ...
  allowed_system_streams_closed:true,
...
Rows 1–12 are the complete file/process symbol coverage, with semantic operation groups frozen by the operation matrix: `parent-walk` and `post-delete-reopen` use their distinct `NtCreateFile` wrappers; `query-file-metadata`, `query-streams` and `enumerate-parent` use `query-file-information-v1`; `rename-no-replace` and `delete-disposition` use `file-mutation-v1`. Rows 13–15 are independent network symbols; `GetExtendedTcpTable` covers listener inventory, while `WSAConnect` and `connect` each cover outbound denial. Both listener detection and outbound denial use the same phase-qualified `STABLE_CODE`, selected by actual phase as `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` or `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED`. There is no separate listener-detection code: a listener row is an outbound-containment failure and carries the identical phase-qualified value. Any raw `OUTBOUND_BLOCKED`, `CONTAINMENT_UNSUPPORTED`, or any `RELISTENER_*`/`LISTENER_DETECTED` alias is invalid. Any module/symbol/wrapper/deny_code/pre-open/post-check mismatch yields the applicable phase-qualified `STABLE_CODE` and `ABORTED_PREFLIGHT` before candidate import or `INVALID_PROTOCOL_DEVIATION` during execution.
  ...
  `opens_volume_root` is a constant of each profile and is exactly `true` for `parent-directory-open` only, and `false` for the other
  ten profiles. `parent-directory-open` therefore always has `STREAM_SCOPE=volume-root`, and every other profile is `file` or
  `directory` according to its `leaf_kind`. No other assignment is representable; any profile whose frozen `opens_volume_root`
  disagrees with this sentence, or any `opens_volume_root=true` on a `leaf_kind=file` profile, is `INVALID_PROTOCOL_DEVIATION`.
  ...
  - `FILE_OPERATION`：`{operation_id:enum[parent-walk,create-directory,create-file,open-file,read-file,write-file,append-file,flush-file,readback-file,
    ...
    stream_reverify:enum[required,not-applicable],stream_scope:oneOf[literal["derived-from-profile"],STREAM_SCOPE],
    stream_query_source:oneOf[literal["not-applicable"],{information_class:"FileStreamInformation",api:"NtQueryInformationFile",handle_source:enum[root-directory-handle,opened-file-handle]}],
  ...
  `stream_reverify` and `stream_scope` and `stream_query_source` are constants of each file operation and are exactly:
  `stream_reverify=required` and `stream_scope="derived-from-profile"` for all sixteen file operations, without exception; the
  `stream_scope` actually applied is the `STREAM_SCOPE` the operation's frozen `profile_id` derives under §2.2, so the closed
  operation matrix and the §2.2 derivation can never disagree. `stream_reverify=not-applicable` is unrepresentable for
  `FILE_OPERATION` and exists only to make the enum closed against future non-file operations. `stream_query_source` is the exact
  object `{information_class:"FileStreamInformation",api:"NtQueryInformationFile",handle_source:opened-file-handle}` for every
  ...
  substitute a different `information_class`, a different `api`, or a `root-directory-handle` source.
...
`NumberOfLinks=1`。该复验对全部十六个 `FILE_OPERATION` 都是强制的：每个 operation 的 `stream_reverify` 必须恰为
`required`，其 `stream_scope` 必须恰为 `derived-from-profile`，并且必须按 `stream_query_source` 在
...
查询、复用其他 operation 的查询结果、或以 `root-directory-handle` 代替 `opened-file-handle` 取得流清单，均为
`INVALID_PROTOCOL_DEVIATION`。
stream 判定先按 §2.2 由该 operation 冻结的 `profile_id` 的 `opens_volume_root` 与 `leaf_kind` 派生唯一 `STREAM_SCOPE`，
再取该 scope 在 `allowed_system_streams` 中的唯一一行，并按该行的 `observation` 判定，规则是闭合的：
...
一律视为 ADS。
在完成上述 allowlist 比对后，`scope=file` 还须满足：必须恰有 unnamed stream `:$DATA`，且除该 unnamed stream 与 allowlist
中 scope 为 `file` 的行所允许的 `SYSTEM_RESERVED_STREAM` 外不得有任何 stream 记录；`scope=directory` 与
`scope=volume-root` 允许 unnamed stream 不出现，且除 allowlist 中该 scope 的行所允许的 `SYSTEM_RESERVED_STREAM` 外不得
有任何 stream 记录。
