...
> **适用性（2026-09-22 实测记录）**：第二条断言**只在该点涉及 generation 时适用**。步骤 2 的领域写入是
> 复习记录、**没有 generation**，故当时**只断言第一条**并在测试文件写明第二条为何不适用——不补空转断言。
> 步骤 4 引入发布门禁后第二条才真正适用，并在 `tests/M10/test_generation_publication.py` 里对**每个发布
> crash point** 断言。
...
> **2026-09-22 实施记录**：Runner 已存在（步骤 6），任务集**已物化**为 `tests/M10/frozen_tasks.py`
> （7 个场景），摘要钉死在 `tests/M10/test_runner_evaluation.py`（`196df3dd…`），门禁在 CI 的 `tests/M10` 步骤内。
> **与下表的两处偏差，各自有理由**：① 新增 `generation-publication` 场景——步骤 4 建了发布门禁，它是独立关切；
> ② **没有「provider 不可用」场景**——M10 的 runner **没有 provider 路径**（源码级断言不 import LLM 客户端），
> 故 arm B 对 M10 不适用。把不可达的回退写成用例只会得到一条空转断言，故改为**钉住它不存在**。
