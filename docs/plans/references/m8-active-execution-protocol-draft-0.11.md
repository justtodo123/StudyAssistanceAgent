...
> 文档版本：`draft-0.11`（仅为文档修订号，不是 experiment ID 或 executable protocol ID）。
> 起草日期：2026-09-12，修订日期：2026-09-14。本文为 draft-0.11 修订；本文所有摘要均由外部记录保存，本文不自哈希。
> 修订依据（六）：draft-0.10 的独立 P3 文本审计判定 `REJECTED / stop`，确认三项 schema 阻断。draft-0.11 按
> [`draft-0.11` 修订授权](m8-active-execution-protocol-draft-0.11-authorization-20260914.md)一次性实施 A+B+C：
> （A）把 11 个 gate 与唯一 actor role、independence flags、operations 形成 closed mapping，并机械禁止独立主体与起草方/链上 owner 重合；
> （B）把 P8 的开放 `admission_scope` 收敛为唯一 literal `protocol-p8-decision-only`；
> （C）定义 canonical `sa.m8.text-audit.v1` artifact，闭合其与 P3 verdict/decision/refs 的解析，并在 durable package 增加独立 `text_audit_member`，保持原 9/19 基数。
> 本修订只改变上述三项及必要交叉引用；不复用 draft-0.10 的 gate/artifact，不预置 P0/P3 结论。
> 修订依据（五）：draft-0.9 在准备 P2 材料时暴露四项遗留缺陷，draft-0.10 按授权记录
...
> 逐项修正（方案 A1 / B1 / C2a / D1a）：
> （A）**P2 的 stream 复验在阶段顺序上不可满足**：§7 要求按 `allowed_system_streams` 判定，而该表属
> `sa.m8.child-allowlist.v1.payload`、即 P5 的产物，P2 时不存在，因此 P2 在**任何卷上**都只能 fail closed。
> draft-0.10 新增由协议直接规定的 `BINDING_STREAM_ALLOWLIST`，并在 §7 增加按门禁阶段选择容许表的闭合规则。
> （B）`forbidden_history_ids` 被标注为 `ID`（不允许点号），而其 13 个取值全部含点号；draft-0.10 将两处标注改为
> `SCHEMA_ID`，**不改动任何取值**。
> （C）`forbidden_history_ids` 虽标注 `order=value`，但按该 comparator 的定义（UTF-8 bytes）算得的顺序为
> `v1, v10, v11, …, v2, …`，而 draft-0.5 与 draft-0.9 两代记录实际采用数字序；draft-0.10 把该字段改为携带
> 显式 `ordinal` 的 object 数组并用 `order=key(ordinal)`，**不改动 `order=value` 自身定义**（该 comparator 另有
> 29 处字段在用）。
> （D）**协议本体未受行尾保护**，使链根摘要随检出环境漂移（同一内容工作区 CRLF 为 `162c9047…` vs 仓库 LF 为
> `6ccebc47…`）；draft-0.10 将 `draft-0.9` 与 `draft-0.10` 逐文件锁定为 LF，使其工作区字节恒等于仓库字节，
> 并据此在 LF 口径上重算 `reviewed_protocol_sha256`。
> **本项不修改 `draft-0.9` 的任何内容字节**（其 LF 摘要仍为 `6ccebc47…`）；它只改变工作区的检出字节口径，
> 使摘要不再因平台而异。`draft-0.5` 作为已冻结历史**不在锁定范围内**（授权记录已否决 D1b），其 3 条记录原样保留。
> `draft-0.6` 至 `draft-0.9` 保持历史，不得 binding 或授权。协议字节的任何变化都会改变 `protocol_blob_sha256`，
> 所有绑定旧摘要的 P0/P1 记录均因此失效，必须在新摘要上重新形成 P0、P1 与 P2。
> 修订依据（一）：draft-0.5 的 stream 约束假定 volume root、directory 与 file 只暴露 unnamed stream，但 Windows 的
> `:sguard:$DATA` 系统保留流会出现在卷根与目录上，使具备该 OS 安全特性的机器上不存在任何可通过 P2 的卷。
> draft-0.6 起将系统保留流从用户 ADS 中分离，并以显式、闭合、可机械复验的 allowlist 处理。
> 修订依据（二）：draft-0.6 的 P0 技术文字审查判定 `RETURNED_FOR_REVISION / P0_NOT_ACCEPTED`，draft-0.7 修正其四项
> 机械缺陷：类型归属与引用位置、volume/device root 判定域缺失、基数从自由区间改为派生常量、interposition ADS 禁令与
> stream 查询信息类的遗漏。
> 修订依据（三）：draft-0.7 的 P0 技术文字审查判定 `RETURNED_FOR_REVISION / P0_NOT_ACCEPTED`，draft-0.8 修正其三项
> 机械缺陷：`STREAM_SCOPE=volume-root` 的判定依据引用了语义不匹配的 `authoritative_handle_source`；
> `allowed_system_streams.observation` 被声明但无任何校验点消费；§7 要求的逐操作 stream 复验在闭合操作矩阵中无法表达。
> 修订依据（四）：draft-0.8 的 P0 技术文字审查判定 `RETURNED_FOR_REVISION / P0_NOT_ACCEPTED`，draft-0.9 修正其三项
> 机械缺陷：scope 被错误地固化为 profile 常量，而 `parent-walk` 逐级打开 volume root 与普通目录，一个 profile 无法
> 承载两种对象；`stream_scope` 为单一标量，无法表达 per-open 派生；`stream_query_source.handle_source` 允许
> `root-directory-handle`，与 §7 正文冲突。draft-0.9 同时删除 `allowed_system_streams_closed` 装饰字段与不可达的
> `stream_reverify=not-applicable`。
...
- `DEC`：canonical finite decimal string `-?(0|[1-9][0-9]*)\.[0-9]{9}`，范围 `[-10^18,10^18]`，禁止 negative zero。所有输入先以任意精度计算，再 half-even 舍入到恰好九位小数；validator 只接受该结果的 canonical 九位表示，不接受 `1`、`1.2`、`1.20`、`-1` 等可归一化别名。例如 `1.000000000`、`1.200000000`、`0.000000000`，inapplicable sentinel 唯一为 `-1.000000000`。`canonical_dec(x)` 是唯一用于 digest、ID、equation operand 和比较序列化的表示。
...
- `STREAM_SCOPE`：`enum[file,directory,volume-root]`，是一个三值闭合枚举，不是 `leaf_kind` 的别名。
  `file` 与 `directory` 对应该次 open 的 `leaf_kind`；`volume-root` 表示该次 open 打开的对象本身就是 volume/device root，
  它既不是 `file` leaf 也不是 `directory` leaf，因此拥有独立取值。
  `STREAM_SCOPE` 是 **per-open** 的：它由该次 open 实际打开的那个对象判定，而不是由任何 profile 常量决定，因为同一个
  profile（尤其 `parent-directory-open`）会在一次 `parent-walk` 中依次打开 volume/device root 与沿途各级普通目录，
  这些对象必须各自得到自己的 scope。判定顺序是闭合的两步，先判卷根、再判 leaf：
  第一步，若本次 open 得到的 handle 是本次 walk 的起始卷根 handle，或该 handle 与本次 walk 记录在
  `volume_root` 的 `FILE_IDENTITY` 逐字段相等（`volume_serial`、`filesystem`、`file_id`、`acl_sha256` 全部相等，
  其中 `file_id` 由 `NtQueryInformationFile(FileIdInformation)` 在 `opened-file-handle` 上取得，`volume_serial` 与之
  一并返回），则本次 open 的 scope 为 `volume-root`；
  第二步，否则按本次 open 的 `leaf_kind` 判定：`leaf_kind=file` 取 `file`，`leaf_kind=directory` 取 `directory`。
  两步互斥且穷尽，因此每次 open 都恰有一个 scope。`leaf_kind=file` 的 open 不可能得到 `volume-root`；若某个 file open
  的 handle 与卷根 `FILE_IDENTITY` 相等，或 `NtQueryInformationFile(FileIdInformation)` 失败、返回的 `file_id` 不是
  `HEX32`、或无法完成与卷根的逐字段比对，本次 open 即 fail closed 并映射为 `ABORTED_PREFLIGHT`。
  卷根身份是每次 walk 的事实，不是 profile 的属性：`NT_OPEN_PROFILE` 不再携带任何表示 volume root 的字段，walk 的
  起始卷根 handle 与其 `FILE_IDENTITY` 必须在该次 walk 开始时先取得并冻结，之后每一次 open 都与之比对。
- `SYSTEM_RESERVED_STREAM`：闭合枚举，当前唯一成员为 `sguard`，序列化形式为 string `"sguard"`，对应 Windows 上的
  `:sguard:$DATA`。该枚举只描述由操作系统组件在卷、目录或文件上维护、且实验进程无权创建、写入或删除的数据流。
  它的每个成员都必须按 `STREAM_SCOPE` 显式列入一份**闭合容许表**才被允许；未列入的 named stream 一律是 ADS。
  该枚举是闭合的，不是开放集合：新增成员唯一地通过一次协议修订完成，并因此改变 `protocol_blob_sha256`、使所有旧摘要
  的 P0/P1/P2 记录失效。`SYSTEM_RESERVED_STREAM` 不使任何 stream 免于逐次复验，也不授权实验进程创建、改名或删除这些流。
- `BINDING_STREAM_ALLOWLIST`：**P2 阶段的内建容许表**，由协议直接规定，不引用任何 artifact。
  取值为 `A<{scope:STREAM_SCOPE,stream:SYSTEM_RESERVED_STREAM,observation:enum[absent-if-empty,present-and-reverified]};3..3;order=key(scope);unique=key(scope)>`，
  逐字为：
  `[{"scope":"volume-root","stream":"sguard","observation":"absent-if-empty"},
  {"scope":"directory","stream":"sguard","observation":"absent-if-empty"},
  {"scope":"file","stream":"sguard","observation":"absent-if-empty"}]`。
  **取值理由（重要，不得改动）**：三个 scope 均取 `absent-if-empty`。该取值在两种环境下都成立：
  卷不暴露该流时不因缺失而失败，暴露时仍须与允许集合一致。若取 `present-and-reverified`，则要求该流
  “必须恰出现一次”，反使**不暴露该流的卷无法通过 P2**——依据 2026-09-13 的实测，本机 `C:` 卷上的
  6 个目录均不暴露 `sguard`，而 `D:` 卷暴露；两种环境都是合规的合法事实，协议不得偏向其一。
  这使 P2 不再依赖“探针恰好在暴露该流的卷上”这一不可控前提。
  该表与 `sa.m8.child-allowlist.v1.payload.allowed_system_streams` **同构**（同类型、同基数、同唯一性约束），
  但**二者不得互相替代**：见 §7 的阶段选择规则。该表只适用于 P2；不因它存在而改变 P5 的 allowlist 语义，
  也不改变 `allowed_system_streams` 的类型定义。
...
- `STABLE_CODE`：`CODE` constrained to one of the exact 72 phase-qualified failure/status values in the closed status-map artifact; the mapping `(source_domain,phase,raw_status) ↔ stable_code` is bijective in both directions, so no stable code is reused by a second tuple and every applicable phase mapping is explicit rather than inferred from a raw status. It never includes `NONE`.
...
- `TARGET_SLOT`：`oneOf[TARGET,literal["not-applicable"]]`；only P7A selected-target slots use this union. A slot is either the exact reserved TARGET or the single sentinel; no other placeholder is valid.
- `MUTATION_VALUE`：闭合 discriminated union：`{kind:"code",value:CODE}` 或 `{kind:"identity",value:ID}`。variant 之外的字段禁止出现；`original_value` 与 `replacement_value` 各自可以使用任一 variant，但两个完整 typed canonical object 不得相等。`kind` 是唯一 discriminator，因此即使字符长度相同也不得跨 variant 推断。`kind:"code"` 只承载大写 CODE；`kind:"identity"` 只承载小写 ID，包括 `id-<kind>-` 加完整 64 hex 的固定 identity literal。
- `ABORT_TERMINAL`：闭合 enum，且顺序即比较顺序：`ABORTED_PREFLIGHT,ABORTED_RUNTIME,INVALID_PROTOCOL_DEVIATION,REJECTED_INPUT_AUDIT,REJECTED_RAW_EVIDENCE`。
  它是 abort cleanup 成功时唯一允许落定的终态集合；P8/P9、`ABORTED_CLEANUP_VERIFIED`、`VALID_EVIDENCE_NOT_PUBLISHED` 与 `CLEANUP_INCOMPLETE` 均不在其中。
- `ABORT_TRIGGER`：闭合 enum，且顺序即比较顺序：`ABORTED_PREFLIGHT,ABORTED_RUNTIME,INVALID_PROTOCOL_DEVIATION,REJECTED_INPUT_AUDIT,REJECTED_RAW_EVIDENCE,VERIFICATION_TIMEOUT,REPORT_TIMEOUT`。
  它是 abort cleanup 的唯一直接触发集合。
- `MUTATION_SPEC`：`{mutation_id:FAULT_FIXTURE_ID,encoding:"canonical-json",target_field:ID,original_value:MUTATION_VALUE,replacement_value:MUTATION_VALUE}`。registry 的每一行都必须完整写出全部五个字段，`mutation_id` 逐字等于该行 `fixture_id`，且 `replacement_value` 与 `original_value` 不相等。其 canonical bytes 是该完整对象按 `sa-json-c14n-v1` 产生的 UTF-8 JSON bytes，绝不从 prose 推断。
...
- `sa.m8.experiment-identity.v1.payload`：
  ...
  forbidden_history_ids:A<{id:SCHEMA_ID,ordinal:INT[0,12]};13..13;order=key(ordinal);unique=key(id)>}`。
...
- `sa.m8.text-audit.v1.payload`：
  `{audit_id:ID,audited_protocol_path:REPO_PATH,audited_protocol_sha256:HEX64,binding_ref:REF,
  verdict:enum[VERIFIED,REJECTED],finding_ids:A<ID;0..64;order=value;unique=value>,
  object_bindings:A<{object_role:enum[protocol,repository-binding],logical_name:LOGICAL_NAME,sha256:HEX64};2..2;
  order=key(object_role);unique=key(object_role)>}`。
  canonical logical name 必须逐字为 `external-artifacts/text-audits/<audit_id>.json`，其中 `<audit_id>` 等于 payload
  的 `audit_id`；reader 必须从 closed payload 重新派生并比较该 logical name。其他目录、Markdown 或非 canonical JSON
  均不得作为 `text_audit_ref`。
  `object_bindings` 必须恰有按 enum 顺序排列的 `protocol` 与 `repository-binding` 各一项；缺失、重复、第三角色或错序均拒绝。
  `binding_ref.schema_id` 必须为 `sa.m8.repository-binding.v1`，并解析到一个 canonical repository binding；
  `audited_protocol_path`/`audited_protocol_sha256` 与该 binding 的 protocol identity 逐字一致。`protocol` object binding
  的 logical name/digest 等于 audited protocol；`repository-binding` object binding 的 logical name/digest 等于
  `binding_ref`。`verdict="VERIFIED"` 当且仅当 `finding_ids=[]`；`verdict="REJECTED"` 当且仅当 findings 非空。
  envelope schema、logical-name binding、canonical bytes、digest、REF resolution 或上述一致性任一失败即
...
- `sa.m8.child-allowlist.v1.payload`：
  ...
  open_profiles:A<NT_OPEN_PROFILE;11..11;order=key(profile_id);unique=key(profile_id)>,
  ...
  allowed_system_streams:A<{scope:STREAM_SCOPE,stream:SYSTEM_RESERVED_STREAM,
  observation:enum[absent-if-empty,present-and-reverified]};3..3;order=key(scope);unique=key(scope)>,
  interposition:{required:true,coverage:"all-allowlisted-file-process-network-symbols",symbols:A<INTERPOSITION_SYMBOL;15..15;order=ordinal;unique=key(module,symbol,wrapper)>,resolver_policy:"frozen-import-table-no-dynamic-resolution",unsupported_code:"PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED"},
  ...
  `INTERPOSITION_SYMBOL` exact fields：`{module:enum[ntdll.dll,kernel32.dll,ws2_32.dll,iphlpapi.dll],symbol:enum[NtCreateFile,NtOpenFile,NtReadFile,NtWriteFile,FlushFileBuffers,NtQueryInformationFile,NtQuerySecurityObject,NtSetInformationFile,NtClose,CreateProcessW,TerminateProcess,GetExtendedTcpTable,WSAConnect,connect],wrapper:ID,deny_code:STABLE_CODE,pre_open_check:enum[none,root-handle,opened-handle,control-handle],post_open_identity_check:BOOL}`。`symbols` 必须恰好是下述 15 行，顺序即 `order=ordinal`；network 的三个原生 API 各占一个独立 row，且 `(module,symbol,wrapper)` 与 `wrapper` 各自唯一：

| # | module | symbol | wrapper | pre_open_check | post_open_identity_check | deny_code |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | ntdll.dll | NtCreateFile | parent-walk-v1 | root-handle | true | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 2 | ntdll.dll | NtCreateFile | post-delete-reopen-v1 | root-handle | true | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 3 | ntdll.dll | NtOpenFile | readback-file-v1 | root-handle | true | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 4 | ntdll.dll | NtReadFile | read-file-v1 | opened-handle | false | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 5 | ntdll.dll | NtWriteFile | write-file-v1 | opened-handle | false | PROTOCOL_RUNTIME_INVALID_PROTOCOL_DEVIATION |
| 6 | kernel32.dll | FlushFileBuffers | flush-file-v1 | opened-handle | false | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 7 | ntdll.dll | NtQueryInformationFile | query-file-information-v1 | opened-handle | true | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 8 | ntdll.dll | NtQuerySecurityObject | query-security-v1 | opened-handle | true | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 9 | ntdll.dll | NtSetInformationFile | file-mutation-v1 | opened-handle | true | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 10 | ntdll.dll | NtClose | close-handle-v1 | none | false | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 11 | kernel32.dll | CreateProcessW | create-process-v1 | control-handle | false | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 12 | kernel32.dll | TerminateProcess | terminate-process-v1 | control-handle | false | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 13 | iphlpapi.dll | GetExtendedTcpTable | listener-inventory-v1 | control-handle | false | PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED |
| 14 | ws2_32.dll | WSAConnect | outbound-wsaconnect-deny-v1 | control-handle | false | WATCHDOG_RUNTIME_OUTBOUND_BLOCKED |
| 15 | ws2_32.dll | connect | outbound-connect-deny-v1 | control-handle | false | WATCHDOG_RUNTIME_OUTBOUND_BLOCKED |

Rows 1–12 are the complete file/process symbol coverage, with semantic operation groups frozen by the operation matrix: `parent-walk` and `post-delete-reopen` use their distinct `NtCreateFile` wrappers; `query-file-metadata`, `query-streams` and `enumerate-parent` use `query-file-information-v1`; `rename-no-replace` and `delete-disposition` use `file-mutation-v1`. Rows 13–15 are independent network symbols; `GetExtendedTcpTable` covers listener inventory, while `WSAConnect` and `connect` each cover outbound denial. Both listener detection and outbound denial use the same phase-qualified `STABLE_CODE`, selected by actual phase as `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` or `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED`. There is no separate listener-detection code: a listener row is an outbound-containment failure and carries the identical phase-qualified value. Any raw `OUTBOUND_BLOCKED`, `CONTAINMENT_UNSUPPORTED`, or any `RELISTENER_*`/`LISTENER_DETECTED` alias is invalid. Any module/symbol/wrapper/deny_code/pre-open/post-check mismatch yields the applicable phase-qualified `STABLE_CODE` and `ABORTED_PREFLIGHT` before candidate import or `INVALID_PROTOCOL_DEVIATION` during execution.

  `NT_OPEN_PROFILE` exact fields：`{profile_id:enum[parent-directory-open,child-directory-create,child-directory-open,child-file-create,child-file-open,child-file-append,child-file-readback,enumeration-open,rename-source-open,delete-source-open,post-delete-reopen],api:"NtCreateFile",leaf_kind:enum[file,directory],disposition:enum[FILE_CREATE,FILE_OPEN],desired_access:ASCII[1,256],
  ...
  child-file-create 与 child-file-append 是仅有的两个携带 `FILE_APPEND_DATA` 的 profile；`append-file` 必须使用 `child-file-append`，
  其写入语义由下述独立 append rule 冻结，而不得复用 create profile；
  ...
  `child-file-append=(file,FILE_OPEN,FILE_APPEND_DATA|FILE_READ_ATTRIBUTES|FILE_WRITE_ATTRIBUTES|DELETE|SYNCHRONIZE,FILE_NON_DIRECTORY_FILE)`；
  ...
  Profiles carry no scope constant. `STREAM_SCOPE` is derived per open under §2.2 from the object that open actually obtained,
  so `parent-directory-open` — which `parent-walk` uses once for the volume/device root and then once per walked component —
  yields `volume-root` for its first open and `directory` for every subsequent open. Any artifact that freezes a single scope
  for a whole profile, any implementation that applies one scope to all opens of a walk, and any comparison that decides the
  root case from a profile field rather than from the opened handle's own identity is `INVALID_PROTOCOL_DEVIATION`.
  ...
  streams that are not listed in the currently effective stream allowlist (§2.1 `BINDING_STREAM_ALLOWLIST` at P2,
  `allowed_system_streams` from P5 on; §7 fixes the selection), path-based fallback APIs, unlisted process launch or network APIs, and any operation
  bypassing the allowlisted wrapper are forbidden; absent coverage for any one operation yields `PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED`
  and `ABORTED_PREFLIGHT`. A `SYSTEM_RESERVED_STREAM` explicitly listed in the currently effective stream allowlist is not an alternate data stream for the
  purpose of this sentence; it remains subject to the per-operation stream re-verification of §7 and may not be created, renamed or deleted.
  ...
  - `FILE_OPERATION`：`{operation_id:enum[parent-walk,create-directory,create-file,open-file,read-file,write-file,append-file,flush-file,readback-file,
    ...
    child-file-create,child-file-open,child-file-append,child-file-readback,enumeration-open,rename-source-open,delete-source-open,post-delete-reopen],
    ...
    information_class:enum[none,FileStreamInformation,FileRenameInformationEx,FileDispositionInformationEx],authoritative_handle_source:enum[root-directory-handle,opened-file-handle],root_handle_required:true,
    stream_reverify:"required",stream_scope_per_open:"required",
    stream_query_source:oneOf[literal["not-applicable"],{information_class:"FileStreamInformation",api:"NtQueryInformationFile",handle_source:"opened-file-handle"}],
    rename_destination:oneOf[literal["not-applicable"],{destination_parent_binding:REF,destination_leaf:TARGET_COMPONENT,
    destination_parent_identity:FILE_IDENTITY,source_identity_before:FILE_IDENTITY,source_identity_after:FILE_IDENTITY}],
  ...
  `append-file→child-file-append/NtWriteFile/opened-file-handle`, `flush-file→child-file-open/FlushFileBuffers/opened-file-handle`,
  ...
  `query-file-metadata→child-file-open/NtQueryInformationFile/none/opened-file-handle`,
  `query-streams→child-file-open/NtQueryInformationFile/FileStreamInformation/opened-file-handle`,
  ...
  `stream_reverify` and `stream_scope_per_open` are literal constants of `FILE_OPERATION` and are exactly `"required"` for all
  sixteen file operations, without exception and with no enum alternatives; a `FILE_OPERATION` that omits either field, or that
  carries any other value, is `INVALID_PROTOCOL_DEVIATION`. `stream_scope_per_open="required"` means every individual open,
  create, rename, delete and reopen performed by the operation must derive its own `STREAM_SCOPE` from the object that open
  actually obtained under §2.2, and must re-verify that object's streams under that scope. An implementation that derives one
  scope per operation, caches a scope across the opens of a walk, or reuses one open's stream list for another open is
  `INVALID_PROTOCOL_DEVIATION`. `stream_query_source` is the exact object
  `{information_class:"FileStreamInformation",api:"NtQueryInformationFile",handle_source:opened-file-handle}` for every
  operation except `query-streams`, which carries the identical object; it is `"not-applicable"` only where the operation's own
  `api` is already `NtQueryInformationFile` with `information_class=FileStreamInformation`, in which case that operation's own
  call is the stream query and no second query is required. No operation may omit the re-verification, and no operation may
  substitute a different `information_class`, a different `api`, or any source other than `opened-file-handle`.
  `rename_destination` is the sentinel for every operation except `rename-no-replace`, and for `rename-no-replace` it carries the exact
  `{destination_parent_binding:REF,destination_leaf:TARGET_COMPONENT,destination_parent_identity:FILE_IDENTITY,source_identity_before:FILE_IDENTITY,source_identity_after:FILE_IDENTITY}`
  object, whose `destination_parent_binding` must be a verified `sa.m8.parent-binding.v1` REF whose resolved parent equals `destination_parent_identity`,
  and which requires same-volume, no-replace, destination-absence, and identity equality before/after. A rename row with the sentinel, or any other row
  with the object, is `INVALID_PROTOCOL_DEVIATION`. The 20 operation rows are exactly the 16 file rows, one close row,
  two process rows, and one network row. The network row's three API alternatives are independently covered by interposition rows 13–15; `GetExtendedTcpTable` supplies listener inventory and `WSAConnect` plus `connect` supply outbound denial. Any `WSAConnect` or `connect` call is intercepted before network access and maps to the phase-qualified `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` or `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED`; a listener row carries the same phase-qualified value, and no `RELISTENER_*` or `LISTENER_DETECTED` alias may be produced, serialized, or expected. Any missing discriminator field, mismatched matrix value, or unlisted operation fails closed.
...
| --- | --- | --- | --- |
| P0 | `[]` | `{gate_id:"P0",review_kind:"P0_TECHNICAL_SCOPE_REVIEW",drafting_party_name:ASCII[1,128],finding_ids:A<ID;0..64;order=value;unique=value>}` | `P0_TECHNICAL_SCOPE_WORDING_ACCEPTED_ONLY/request-p1` or `P0_NOT_ACCEPTED/stop` |
| P1 | `[P0]` | `{gate_id:"P1",identity_ref:REF,forbidden_history_ids:A<{id:SCHEMA_ID,ordinal:INT[0,12]};13..13;order=key(ordinal);unique=key(id)>}` | `AUTHORIZED/request-p2` or `NOT_AUTHORIZED/stop` |
...
Gate identity, actor role, independence flags and operation set form one closed mapping; none is independently selectable:
P0/P3/P5=`independent-reviewer,true,true,[review]`; P7=`independent-verifier,true,true,[verify]`;
P1=`owner,false,false,[identity]`; P2=`owner,false,false,[binding]`; P4=`owner,false,false,[acquire,prepare]`;
P6=`owner,false,false,[execute]`; P7A=`owner,false,false,[cleanup,publish]`; P8=`owner,false,false,[admit]`;
P9=`owner,false,false,[select]`. Any role, flag, operation value, cardinality or order mismatch is schema failure and gate stop.
`independence.basis` is explanatory text only and cannot repair a mismatch or prove independence.

Independent-subject non-overlap is also mechanical. P0 payload carries required `drafting_party_name`; P0 `actor.name` must be
byte-different from it. For P3/P5/P7, resolve the unique predecessor chain back to P0, collect every `actor.name` whose closed
mapping role is `owner`, and require the current independent actor name to differ byte-for-byte from every collected owner name.
Missing chain records, duplicate/spliced predecessors, inability to resolve an actor, equality with drafting party/owner, or an
attempt to rely on aliases or `basis` is `INVALID_PROTOCOL_DEVIATION`. P0 has no owner predecessor and therefore uses its explicit
drafting-party field. No unbound `principal_id` or prose identity substitute is allowed.
...
`selected_write_targets` is a five-slot array in fixed order and is a total function of the P7A decision:
`AUTHORIZED` maps to `[package,normal,not-applicable,not-applicable,not-applicable]`, while either
`PUBLICATION_REFUSED` or `NOT_AUTHORIZED` maps to `[not-applicable,not-applicable,nonpublication,not-applicable,not-applicable]`.
Abort, cleanup-failure, and package-failure are not P7A decisions and have no P7A target tuple. P7A contains no
`written_targets` field: it records authorization only. Actual writes are facts emitted after the operation by detached
writer-result artifacts, and a target is usable only when its corresponding writer result is a verified success variant.
A slot not in the decision's exact function is the sentinel, and any real target in another slot is `INVALID_PROTOCOL_DEVIATION`. P8/P9 have empty `write_targets` and read exactly their
package/receipt/writer-result authority REFs. P8 payload `admission_scope` has the single literal
`"protocol-p8-decision-only"`; every other value is invalid. It records only this protocol's P8 decision and cannot create, alter,
replace or imply project M8 registry/stage admission, implementation authorization or backend selection; P9 remains the sole
protocol-internal backend-selection gate. P8 cannot substitute for `docs/PLAN.md`, formal admission policy/registry or owner approval.

P3 has empty `write_targets` and its `read_refs` must contain exactly three REFs: the inspected P2 gate, that P2 payload's
repository binding, and the canonical `sa.m8.text-audit.v1` artifact named by `text_audit_ref`; `text_audit_ref.schema_id`
must be exactly `sa.m8.text-audit.v1`. P3 `binding_ref`, the audit
artifact's `binding_ref`, and P2's repository-binding REF must be byte-identical; the artifact protocol identity must equal the
successor binding's protocol identity. Audit `VERIFIED` uniquely maps to P3 `VERIFIED/request-p4`; audit `REJECTED` uniquely maps
to P3 `REJECTED/stop`. Any verdict/decision/action/reference mismatch is `INVALID_PROTOCOL_DEVIATION`.

All records bind the same protocol path and digest. Missing refs, extra refs or targets, wrong order, wrong predecessor, digest
mismatch, or scope expansion is a gate stop.
...
The writer-result reference contract is variant-specific and closed. A normal cleanup receipt, a nonpublication cleanup receipt, and an abort cleanup receipt use `receipt_writer_result_ref`, which MUST resolve to the success variant. Its binding, role, authorization, state transition, byte digest/count, create, flush, close, reopen, and readback facts equal the actual receipt; omission or mismatch is `INVALID_PROTOCOL_DEVIATION`. A `cleanup-failure-record` carries two distinct references. `attempted_receipt_writer_result_ref` attests the receipt write that failed and MUST resolve to the failure variant of the attempted receipt target. `failure_record_writer_result_ref` attests the failure record's own write and MUST resolve to the success variant of the failure-record target, because the failure record is durable only after its own create/write/flush/close/reopen/readback succeeded; the two reference fields are not interchangeable. The failure variant has no target identity, artifact digest/count, success booleans, or artifact REF. Its `status` is `WRITE_FAILED`, its `failure_stage` and phase-qualified `STABLE_CODE` are the only failure result fields, and branch triggers and terminal states use their separately declared enums. Success has `stable_code="NONE"`; the variants may not mix fields.
...
generator_source_sha256:HEX64,oracle:"numpy-float64-exhaustive",score_tolerance:"0.000010000",tie_tolerance:"0.000001000"}`。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `measured-hit` | `hit` | `unfiltered` | `none` | `false` | role ordinal: 1/3/5 | `-1.000000000` | nonempty, exact `top_k` |
| `measured-filtered-hit` | `filtered-hit` | `filtered` | `exact` | `false` | role ordinal: 1/3/5 | `-1.000000000` | nonempty, exact `top_k`, every ID passes filter |
| `measured-deny-owner` | `deny-owner` | `deny` | `deny-owner` | `false` | `5` | `0.999999000` | empty, `oracle_empty=true` |
| `measured-deny-source` | `deny-source` | `deny` | `deny-source` | `false` | `5` | `0.999999000` | empty, `oracle_empty=true` |
| `warmup` | `warmup` | `unfiltered` | `none` | `true` | `1` | `-1.000000000` | nonempty, exact `1` |
...
sorts by descending unrounded score then ascending chunk ID. `score_tolerance=0.000010000` is used only to compare a recomputed
finite score with the stored score: absolute error must be at most that value. `tie_tolerance=0.000001000` never changes ranking;
...
- `sa.m8.network-inventory-evidence.v1.payload`：`{capture_phase:enum[preflight,post-child-exit],connections:A<{local:ASCII[1,128],remote:ASCII[1,128],state:enum[CLOSED,LISTENING,SYN_SENT,SYN_RECEIVED,ESTABLISHED,FIN_WAIT1,FIN_WAIT2,CLOSE_WAIT,CLOSING,LAST_ACK,TIME_WAIT,DELETE_TCB],owner_pid:INT[0,4294967295]};0..0;order=key(local,remote,state,owner_pid);unique=key(local,remote,state,owner_pid)>,api:"GetExtendedTcpTable",outbound_attempts:INT[0,0],egress_bytes:INT[0,0]}`。
  它也是 `pred-network-zero-v1` 的 `egress_bytes` 的唯一来源：该值由 `GetExtendedTcpTable` 与三个 network wrapper 的审计计数重算，不接受 prose、其他 gate 的 `passed` 或任何标量摘要替代。
- `sa.m8.interposition-ledger-evidence.v1.payload`：`{resolver_policy:"frozen-import-table-no-dynamic-resolution",symbols:A<INTERPOSITION_SYMBOL;15..15;order=key(module,symbol,wrapper);unique=key(module,symbol,wrapper)>,operation_bindings:A<{operation_id:CONTAINMENT_OPERATION_ID,symbol:enum[NtCreateFile,NtOpenFile,NtReadFile,NtWriteFile,FlushFileBuffers,NtQueryInformationFile,NtQuerySecurityObject,NtSetInformationFile,NtClose,CreateProcessW,TerminateProcess,GetExtendedTcpTable,WSAConnect,connect],wrapper:ID};20..20;order=key(operation_id);unique=key(operation_id)>,production_write_attempts:A<{operation_id:enum[create-file,append-file],target_class:enum[package,normal-receipt,nonpublication-receipt,abort-receipt,failure-receipt,preparation-manifest,preparation-fixture,preparation-source,preparation-index,preparation-report],performing_child_role:enum[coordinator,provisioner,query-worker,lifecycle-worker,fault-worker,report-writer,package-writer,cleanup-writer],allowed:BOOL,attempt_count:INT[0,100000]};20..20;order=key(operation_id,target_class);unique=key(operation_id,target_class)>,production_write_count:INT[0,1000000],unlisted_calls:INT[0,0],dynamic_resolution_used:false}`。
  它也是 `pred-production-write-zero-v1` 的 `production_write_count` 的唯一来源：该值恰为 `production_write_attempts` 中 `allowed=false` 条目的 `attempt_count` 之和，
  由 verifier 从该数组重算，不从其他 gate 的 `passed`、日志行数或标量摘要复制；`allowed=true` 条目只对应被授权的 publication/preparation 写入，不参与该和。
  `production_write_attempts` 是下列 20 行的闭合集：`operation_id` 依次为 `append-file`、`create-file`，每个 operation 下的
  `target_class` 均依次为 `abort-receipt,failure-receipt,nonpublication-receipt,normal-receipt,package,preparation-fixture,preparation-index,preparation-manifest,preparation-report,preparation-source`。
  每个 tuple 恰出现一次。前五个 publication target class 的唯一 performing role 分别为
  `cleanup-writer,cleanup-writer,cleanup-writer,cleanup-writer,package-writer`；后五个 preparation target class 均为
  `provisioner`。在存在对应 P4/P6/P7A 授权且 operation 与 target class 的冻结写入方式一致时 `allowed=true`，否则
  `allowed=false`；缺失授权、worker role 不符、未列 target class 或额外 tuple 均 fail closed。`attempt_count` 是该 tuple
  观测到的实际调用数而不是预算。该数组的 2×10=20 行顺序由 `key(operation_id,target_class)` 唯一决定。
...
- `QUERY_METRIC`：`{workload:WORKLOAD,backend:BACKEND,repetition:INT[0,5],query_class:QUERY_CLASS,
  ...
  recall_at_1:DEC,recall_at_3:DEC,recall_at_5:DEC,mrr:DEC,no_hit_precision:DEC}`. For an inapplicable aggregate, the value is the closed sentinel `-1.000000000`: Recall/MRR are applicable only to unfiltered/filtered, and no-hit precision only to deny. `100k-capacity` has no filtered row.
...
`sa.m8.exact-flat-proof.v1.payload` exact fields：`{scope:enum[query,build,lifecycle],backend:BACKEND,workload:WORKLOAD,sample_id:ID,sample_root_target:TARGET,corpus_ref:oneOf[REF,literal["not-applicable"]],query_gold_ref:oneOf[REF,literal["not-applicable"]],provisioning_child_source_sha256:HEX64,catalog_digest:HEX64,index_inventory_digest:HEX64,query_plan_digest:HEX64,vector_index_count:INT[0,0],ann_index_count:INT[0,0],scalar_secondary_index_count:INT[0,0],search_mode:"exhaustive",distance:"cosine",dimension:INT[512,512],index_kind:"flat/no-vector-index",identity_set_sha256:HEX64,metadata_sha256:HEX64,sentinel_parity:true,provision_build_close_readback_verified:true}`。`scope=query` 时 `corpus_ref` 与 `query_gold_ref` 必须为 REF；`scope=build` 或 `scope=lifecycle` 时二者必须为字面量 `"not-applicable"`。SQLite proof 必须将 `index_inventory_digest` 与 `query_plan_digest` 绑定到 linear-scan catalog；LanceDB proof 必须绑定 catalog、index inventory 和 explain plan。

proof coverage 由 canonical `sa.m8.sample-inventory.v1` 机械导出，是单一有序集合：query 40、build 48、lifecycle 3,248，总数恰为 3,336。每个 inventory item 必须且只能导出一个 proof key `{scope,sample_id,sample_root_target}`；任何缺失、重复、额外 key，或同一 target 在不同 scope 复用均 fail closed。全局顺序是 `key(scope,workload,backend,sample_id)`，其中 scope 比较顺序为 `query,build,lifecycle`。`sample_root_target` 必须逐字等于下列 scope-specific 唯一导出结果，不得包含其他 component，也不得按 run、时钟或随机数命名：

`scope=query`：`root_identity_sha256=SHA256(LP("sa-m8-query-sample-root-v1")||LP(corpus_ref.sha256)||LP(query_gold_ref.sha256)||LP(sample_id))`；
`scope=build`：`root_identity_sha256=SHA256(LP("sa-m8-build-sample-root-v1")||LP(sample_inventory_ref.sha256)||LP(sample_id))`；
`scope=lifecycle`：`root_identity_sha256=SHA256(LP("sa-m8-lifecycle-sample-root-v1")||LP(sample_inventory_ref.sha256)||LP(sample_id))`；
`sample_leaf = scope-prefix + root_identity_sha256`，scope-prefix 分别为 `q`、`b`、`l`；
`sample_root_target = TARGET{parent_binding:experiment_parent_binding,components:["samples",scope,sample_leaf],leaf_kind:"directory",leaf_name:sample_leaf}`。

其中 `sample_inventory_ref` 是 execution report 的 `input_refs.sample_inventory`，且其 digest 是 build/lifecycle root preimage 的唯一 inventory commitment。

其中 `experiment_parent_binding` 逐字等于 P2 repository binding 的 `experiment_parent_binding` REF，`target()` 逐字等于 §7 的 `PARENT_BINDING` 导出：`components` 从该 binding 的 verified parent handle 起按 `ordinal` 依次 `child-directory-create`，只有最后一个 component 可在其已不存在时创建。proof 的 `scope`、`sample_id`、`backend`、`workload` 与 `sample_root_target` 必须与其闭包到的 inventory item 逐项相等；query proof 还必须使 `corpus_ref`、`query_gold_ref` 与该 query item 的 manifest REF 相等，另外两类则必须使用 sentinel。由此 3,336 个 target 两两互异并恰为 sample inventory 导出的完整集合。每个 target 必须在 provisioning child 启动前通过 handle identity 与 parent enumeration identity 验证、在 proof 写入前经 readback 复验，且不得跨 inventory item 复用；任何跨 sample root 的 index、cache、已打开 handle 或 mutable state 复用均为 `PROTOCOL_RUNTIME_INVALID_PROTOCOL_DEVIATION`。
...
- `exact_flat_proof_refs:A<REF;0..3336;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>`；`execution_disposition=EXECUTION_FINISHED_PENDING_VERIFICATION` 时必须恰为 3,336，且覆盖 `scope=query` 的 40 个、`scope=build` 的 48 个和 `scope=lifecycle` 的 3,248 个 canonical sample roots；abort/invalid 时只能包含失败前已完成并 durable-readback 的 proof prefix，未完成 sample 不得伪造 proof；每个已有 proof 必须闭包到唯一的 `{scope,sample_id,sample_root_target}`，并按 `key(scope,workload,backend,sample_id)` 排序；缺失、重复、多余或跨 scope 复用均 fail closed；
...
- `exact_flat_proof_ref_set_sha256:HEX64`，其 preimage 是 `SHA256(LP("sa-m8-exact-flat-proof-ref-set-v1")||LP(count)||LP(ref0.sha256)||…||LP(refN-1.sha256))`，`count` 是 `exact_flat_proof_refs` 的长度，`refN` 按该数组的 `key(logical_name,schema_id,sha256)` 顺序展开；
...
`{execution_report:REF,genesis_layer_ref:REF,control_evidence_refs:{environment:REF,resource_watchdog:REF,process_inventory:REF,network_inventory:REF,interposition_ledger:REF},verifier_source_sha256:HEX64,exact_flat_proof_refs:A<REF;3336..3336;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,exact_flat_proof_ref_set_sha256:HEX64,recomputed_refs:A<REF;29..29;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,
...
genesis_member:PACKAGE_MEMBER_REF,member_decoded_bytes:INT[1,268435456],proof_members:A<PACKAGE_MEMBER_REF;3336..3336;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,proof_ref_set_sha256:HEX64,redaction:{raw_vectors_absent:true,credentials_absent:true,absolute_user_paths_absent:true,
full_environment_absent:true,backend_raw_errors_absent:true}}`。

`text_audit_member` 必须在 schema ID、logical name、digest 与 decoded canonical bytes 四方面精确等于 P3
`text_audit_ref`，且不计入、不插入 `gate_members` 或 `input_members`。`gate_members` 仍恰为 9，`input_members` 仍恰为 19；
缺失、重复、放入任一数组、引用不同 audit artifact 或 member bytes/digest 不一致均为 `INVALID_PROTOCOL_DEVIATION`。
...
member。3,336 个 `proof_members` 必须按统一的 `{scope,sample_id,sample_root_target}` identity 恰好覆盖 40 个 query、48 个 build 和 3,248 个 lifecycle sample roots，并逐字等于 execution/verification report 引用的 exact-flat proofs；`proof_ref_set_sha256` 必须逐字等于两份 report 的 `exact_flat_proof_ref_set_sha256`，并按 §6.1 同一 preimage 从 `proof_members` 的 REF 投影独立重算；observation_refs 不作为独立 package members，而由 execution report 的 canonical bytes 和 gate/layer/fault members 闭包承诺，其 raw observation artifacts 在 cleanup 后不可解引用，package 不得声称包含它们。四个 corpus manifest 的 `corpus_jsonl:JSONL_COMMITMENT` 不是 REF；其 `logical_name/schema_id/sha256` 必须逐项等于
...
`sa.m8.cleanup-failure-record.v1.payload`（当 normal、nonpublication 或 abort cleanup receipt 的 CREATE_NEW/write/flush/close/reopen/readback 失败时，在预先冻结的 failure-receipt target 上尝试 CREATE_NEW 写入；它不证明 cleanup 成功）是一个由 `branch` 判别的闭合 union，三个 variant 的精确字段如下，不存在共享骨架之外的字段。判别完全由逐字 `branch` 值机械确定，不依赖任何外部叙述；**恰有一个** branch-dependent 字段：`branch=normal` 携带 `pending_disposition:"CLEANUP_INCOMPLETE"` 且禁止 `pending_terminal`，`branch=abort` 与 `branch=nonpublication` 携带 `pending_terminal:ABORT_TERMINAL` 且禁止 `pending_disposition`。两者同时出现、同时缺失，或出现所属 variant 未列出的任何字段，均为 schema failure；`branch` 不属于 `enum[normal,nonpublication,abort]` 亦为 schema failure。

三个 variant 的完整字段如下。`branch=abort` variant：
`{identity_ref:REF,branch:"abort",trigger:enum[ABORTED_PREFLIGHT,ABORTED_RUNTIME,INVALID_PROTOCOL_DEVIATION,REJECTED_INPUT_AUDIT,REJECTED_RAW_EVIDENCE,VERIFICATION_TIMEOUT,REPORT_TIMEOUT],
pending_terminal:ABORT_TERMINAL,
...
failure_record_target:TARGET,attempted_receipt_writer_result_ref:REF,failure_record_writer_result_ref:REF,before:{entries:INT[0,1000000],bytes:INT[0,17179869184],handles:INT[0,100000],processes:INT[0,16],listeners:INT[0,0],root_exists:BOOL},
...
`branch=normal` variant：
`{identity_ref:REF,branch:"normal",trigger:enum[CLEANUP_TIMEOUT,RECEIPT_CREATE_FAILED,RECEIPT_WRITE_FAILED,RECEIPT_READBACK_FAILED,RECEIPT_VALIDATION_FAILED],
pending_disposition:"CLEANUP_INCOMPLETE",
cleanup_authorization_refs:A<REF;1..2;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,attempted_receipt_target:TARGET,
failure_record_target:TARGET,attempted_receipt_writer_result_ref:REF,failure_record_writer_result_ref:REF,before:{entries:INT[0,1000000],bytes:INT[0,17179869184],handles:INT[0,100000],processes:INT[0,16],listeners:INT[0,0],root_exists:BOOL},
write_failure_code:STABLE_CODE,receipt_write_attempted:true,cleanup_proven:false,final_disposition:"CLEANUP_INCOMPLETE",created_at:TS}`。

`branch=nonpublication` variant：
`{identity_ref:REF,branch:"nonpublication",trigger:enum[P7A_REFUSED,PACKAGE_TIMEOUT,PACKAGE_CREATE_FAILED,PACKAGE_WRITE_FAILED,PACKAGE_READBACK_FAILED,PACKAGE_VALIDATION_FAILED,CLEANUP_TIMEOUT,RECEIPT_CREATE_FAILED,RECEIPT_WRITE_FAILED,RECEIPT_READBACK_FAILED,RECEIPT_VALIDATION_FAILED],
pending_terminal:ABORT_TERMINAL,
cleanup_authorization_refs:A<REF;1..2;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,attempted_receipt_target:TARGET,
failure_record_target:TARGET,attempted_receipt_writer_result_ref:REF,failure_record_writer_result_ref:REF,before:{entries:INT[0,1000000],bytes:INT[0,17179869184],handles:INT[0,100000],processes:INT[0,16],listeners:INT[0,0],root_exists:BOOL},
write_failure_code:STABLE_CODE,receipt_write_attempted:true,cleanup_proven:false,final_disposition:"CLEANUP_INCOMPLETE",created_at:TS}`。

三个 variant 按 §2.3 的规范键序序列化；`pending_terminal` 与 `pending_disposition` 只出现在各自 variant 的规范位置，
不构成对"共享字段集"的补充：任一 variant 中未列出的字段出现即为 schema failure。该 record 不携带任何 package digest、
execution/verification report REF，也不得新增字段来补记它们。

Cross-field combinations are closed：`branch=abort` 只允许其上列 trigger 集合，且 `pending_terminal` 必须逐字等于 `trigger` 经下列唯一映射得到的 abort terminal：`ABORTED_PREFLIGHT→ABORTED_PREFLIGHT`、`ABORTED_RUNTIME→ABORTED_RUNTIME`、`INVALID_PROTOCOL_DEVIATION→INVALID_PROTOCOL_DEVIATION`、`REJECTED_INPUT_AUDIT→REJECTED_INPUT_AUDIT`、`VERIFICATION_TIMEOUT→REJECTED_RAW_EVIDENCE`、`REPORT_TIMEOUT→ABORTED_RUNTIME`、`REJECTED_RAW_EVIDENCE→REJECTED_RAW_EVIDENCE`；任何其他组合均为 schema failure。authorization refs 在 P6 前恰为 `[P4]`、P6 后恰为 `[P4,P6]`。
`branch=normal` 只有在该失败确实终止于 `CLEANUP_INCOMPLETE` 时合法，其 terminate 由 `pending_disposition` 的固定值
`"CLEANUP_INCOMPLETE"` 机械确定并必须逐字等于 `final_disposition`；normal cleanup 只在所有层已 PASS 且达到
`PACKAGE_WRITTEN`/`CLEANUP_IN_PROGRESS` 后发生，因此该 variant 不携带、也不得虚构任何 abort terminal，authorization refs 恰为 `[P6,P7A]`。`branch=nonpublication` 只允许其上列 trigger 集合，`pending_terminal` 为 `{ABORTED_PREFLIGHT,ABORTED_RUNTIME,INVALID_PROTOCOL_DEVIATION}` 之一；若 `trigger=P7A_REFUSED` 则 authorization refs 恰为 `[P6]`，若是 package failure（含 `PACKAGE_TIMEOUT`）则恰为 `[P6,P7A]`，若是 cleanup/receipt failure 则 refs 必须逐字沿用被替代 nonpublication receipt 本应使用的 `[P6]` 或 `[P6,P7A]`。任何未列 branch/trigger/pending-terminal/ref tuple 都是 schema failure。`pending_terminal` 在该 record 的 abort/nonpublication variant 中只作为已冻结的 abort terminal 标签携带，绝不表示已完成 abort cleanup，且 P8/P9、`ABORTED_CLEANUP_VERIFIED`、`VALID_EVIDENCE_NOT_PUBLISHED` 永不出现。attempted receipt 和 failure-record target 必须分别逐字等于适用 P4/P7A target，且互异。`attempted_receipt_writer_result_ref` 必须解析到 attempted receipt target 的 failure variant，`failure_record_writer_result_ref` 必须解析到 failure-record target 的 success variant；任一 REF 缺失、未解析、variant 不匹配或与 target 不符均为 schema failure。两个 writer result 都是 out-of-band attestation：它们逐字绑定各自 target、canonical bytes、CREATE_NEW 结果和 readback，不得由被证明 bytes 引用自身。
...
`{identity_ref:REF,cleanup_authorization_refs:A<REF;1..2;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,receipt_writer_result_ref:REF,receipt_canonical_bytes:INT[1,16777216],pending_terminal:ABORT_TERMINAL,trigger:ABORT_TRIGGER,before:{entries:INT[0,1000000],bytes:INT[0,17179869184],handles:INT[0,100000],processes:INT[0,16],listeners:INT[0,0],root_exists:BOOL},
...
任何残留必须令 `after_zero=false` 且只能为 `CLEANUP_INCOMPLETE`。`trigger` 是该 abort cleanup 的直接原因，`pending_terminal` 是 cleanup 成功时必须逐字落定的 `ABORT_TERMINAL` 最终终态；
...
FAIL 经同一 trigger→abort-terminal 映射产生的 `ABORT_TERMINAL`，且必须与该 abort 在 `sa.m8.cleanup-failure-record.v1.payload` 的 `branch=abort` variant 所携带的 `pending_terminal` 逐字一致；`pending_terminal` 只允许 `ABORT_TERMINAL` 的五个值，禁止 `"none"` 或任何 `TERMINAL_STATE` 成员，尤其禁止 P8/P9、`ABORTED_CLEANUP_VERIFIED`、`VALID_EVIDENCE_NOT_PUBLISHED`；清理前后的 `entries`/`bytes`/`handles`/`processes`/`listeners` 必须全为零且 `root_exists=false`，否则不得创建 abort receipt 并必须转入 `CLEANUP_INCOMPLETE`；`final_disposition=CLEANUP_INCOMPLETE` 永久无出边。
...
`{execution_report_ref:REF,verification_report_ref:REF,identity_ref:REF,cleanup_authorization_refs:A<REF;1..2;order=key(logical_name,schema_id,sha256);unique=key(logical_name)>,publication_failure:enum[P7A_REFUSED,PACKAGE_TIMEOUT,PACKAGE_CREATE_FAILED,PACKAGE_WRITE_FAILED,PACKAGE_READBACK_FAILED,PACKAGE_VALIDATION_FAILED],
...
`OBJ_CASE_INSENSITIVE`、下述 operation-specific 写入 offset 规则、`FlushFileBuffers`、close 顺序和失败码必须在
child allowlist artifact 中逐项冻结，缺项即 preflight abort。写入 offset 规则是闭合的二分：
`write-file` 是 `NtWriteFile` 单次完整写，absolute byte offset 恰为 `0`，且期望写入后文件长度恰等于 payload 长度
（offset≠0、分次写或短写均为 `INVALID_PROTOCOL_DEVIATION`）；
`append-file` 只能是 `FILE_APPEND_DATA` profile `child-file-append` 上的 `NtWriteFile`，byte offset 参数必须是
`FILE_WRITE_TO_END_OF_FILE` 语义（不使用显式 offset），自当前 end-of-file 起追加，单次完整写，
追加后新长度恰为原长度加 payload 长度，且不得覆写、截断或重写任何既有字节；两个 operation 的 payload 均不得为空。
不区分 offset 语义、用显式 offset 模拟 append、或以 `child-file-create` profile 执行 append，均为 preflight abort。

每次 open/create/write/rename/delete/reopen 均复验 volume serial、128-bit file ID、parent enumeration identity、type/tag 和
`NumberOfLinks=1`。该复验对全部十六个 `FILE_OPERATION` 都是强制的：每个 operation 的 `stream_reverify` 与
`stream_scope_per_open` 都必须恰为 `"required"`，并且必须按 `stream_query_source` 在
`NtQueryInformationFile(FileStreamInformation)` 上于 `opened-file-handle` 取得流清单后再判定，唯一例外是自身即以
`NtQueryInformationFile` 加 `FileStreamInformation` 执行 `query-streams` 的操作，该次调用本身即构成流查询。缺失该次
查询、复用其他 operation 或同一次 walk 中其他 open 的查询结果、或以 `root-directory-handle` 或任何非
`opened-file-handle` 的来源取得流清单，均为 `INVALID_PROTOCOL_DEVIATION`。
stream 判定是 per-open 的，并按下述闭合顺序执行。每次 walk 开始时必须先打开并冻结起始卷根 handle 及其在
`volume_root` 的 `FILE_IDENTITY`；此后每一次 open 都先按 §2.2 的第一步判定本次 scope：本次 open 的 handle 是起始卷根
handle，或与冻结的卷根 `FILE_IDENTITY` 在 `volume_serial`、`filesystem`、`file_id`、`acl_sha256` 四个字段上全部相等
（全程 relative open、无 follow，`file_id` 取自本次 open 的 `opened-file-handle`），则 scope 为 `volume-root`；否则按本次
open 的 `leaf_kind` 取 `file` 或 `directory`。因此一次 `parent-walk` 在 volume/device root 上得到 `volume-root`，在沿途每一级
普通目录上各自得到 `directory`，绝不使用同一个 scope 覆盖整次 walk。
得到本次 scope 后，取该 scope 在**当前生效的容许表**中的唯一一行，并按该行的 `observation` 判定，规则是闭合的：
当前生效的容许表按下述两条互斥且穷尽的规则选择，该选择是机械的，不得由实施方选优：
（一）当本次操作属于 **P2**（`sa.m8.repository-binding.v1` 与 `sa.m8.parent-binding.v1` 的生成与复验，
包括为冻结 `repository_commit`、`experiment_parent_binding` 与 `publication_parent_bindings` 所需的全部 `parent-walk`）时，
当前生效的容许表**恰为** `BINDING_STREAM_ALLOWLIST`；此时 `allowed_system_streams` 尚不存在，引用它即为失败，
不得以“该 artifact 未产出”为由豁免或推迟本次复验。
（二）当本次操作属于 **P5 及其后**（`child_allowlist` 已产出）时，当前生效的容许表**恰为**
`sa.m8.child-allowlist.v1.payload.allowed_system_streams`，且**不得**再引用 `BINDING_STREAM_ALLOWLIST`。
两张表的选择只由本次操作所属的门禁阶段决定（该阶段由本次操作的 `authorization_ref` 唯一确定，不得由实施方声明），
不得交叉、不得合并、不得取并集；任何“两表兼容即可通过”的推断均为 `INVALID_PROTOCOL_DEVIATION`。
`observation=absent-if-empty` 表示该 scope 的该流允许完全不出现，出现时必须与下述逐条允许集合一致，不得因缺失而失败；
`observation=present-and-reverified` 表示该 scope 的该流必须在流清单中恰好出现一次，且必须在本次写入后再次查询并确认
其仍然存在，出现零次或出现多次均为失败。未在该 scope 行中列出的 `SYSTEM_RESERVED_STREAM` 与任何其他 named stream
一律视为 ADS。
在完成上述容许表比对后，本次 open 还须按其自身 scope 满足：`scope=file` 必须恰有 unnamed stream `:$DATA`，且除该
unnamed stream 与当前生效容许表中 scope 为 `file` 的行所允许的 `SYSTEM_RESERVED_STREAM` 外不得有任何 stream 记录；
`scope=directory` 与 `scope=volume-root` 允许 unnamed stream 不出现，且除当前生效容许表中该 scope 的行所允许的
`SYSTEM_RESERVED_STREAM` 外不得有任何 stream 记录。这些约束逐次 open 独立成立，不得以一次 open 的结果推断另一次。
（说明：本段及下文所说的“当前生效容许表”，按本节前述阶段选择规则确定；在 P2 即为 `BINDING_STREAM_ALLOWLIST`。）
任何未列入当前生效容许表的 named stream、任何缺失的 unnamed stream、容许表外部观测到的任何 stream 记录、任何
`observation` 判定不成立的流清单，均使本次操作 fail closed 并映射为稳定 CODE；执行前为 `ABORTED_PREFLIGHT`，执行中为
`INVALID_PROTOCOL_DEVIATION`，cleanup 中为 `CLEANUP_INCOMPLETE`。
禁止 symlink、junction、mount point、任何其他 reparse、未列入 allowlist 的 ADS、hard link、跨卷、case-alias collision 和 identity swap。rename 使用
...
handle-relative contract 时才允许；否则实现必须在 candidate import 前写 `PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED` 并 `ABORTED_PREFLIGHT`，
...
`FAULT_REGISTRY_ENTRY` exact fields：`{fixture_id:FAULT_FIXTURE_ID,mutation:MUTATION_SPEC,
...
`mutation` is a `MUTATION_SPEC`, not an opaque fixture label. Each registry row writes the complete five-field object:
`mutation.mutation_id` equals the row `fixture_id` verbatim, `encoding` is the literal `"canonical-json"`, and
`target_field`, `original_value`, and `replacement_value` are present field-for-field in the closed row. The frozen rows are:
`unauthorized-owner={mutation_id:"unauthorized-owner",encoding:"canonical-json",target_field:"owner-metadata-field",original_value:{kind:"identity",value:"id-owner-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},replacement_value:{kind:"identity",value:"id-owner-bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}}`；
`cross-source={mutation_id:"cross-source",encoding:"canonical-json",target_field:"source-metadata-field",original_value:{kind:"identity",value:"id-source-cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"},replacement_value:{kind:"identity",value:"id-source-dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"}}`；
`stale-generation={mutation_id:"stale-generation",encoding:"canonical-json",target_field:"generation-metadata-field",original_value:{kind:"code",value:"GENERATION_CURRENT"},replacement_value:{kind:"code",value:"GENERATION_STALE"}}`；
`stale-snapshot={mutation_id:"stale-snapshot",encoding:"canonical-json",target_field:"snapshot-metadata-field",original_value:{kind:"code",value:"SNAPSHOT_CURRENT"},replacement_value:{kind:"code",value:"SNAPSHOT_STALE"}}`；
`wrong-dimension={mutation_id:"wrong-dimension",encoding:"canonical-json",target_field:"vector-dimension-field",original_value:{kind:"code",value:"DIMENSION_512"},replacement_value:{kind:"code",value:"DIMENSION_384"}}`；
`wrong-vector-profile={mutation_id:"wrong-vector-profile",encoding:"canonical-json",target_field:"vector-profile-field",original_value:{kind:"code",value:"PROFILE_SYNTHETIC_SHAKE256_UNIT_512_F32_V1"},replacement_value:{kind:"code",value:"PROFILE_MISMATCH"}}`；
`tombstone={mutation_id:"tombstone",encoding:"canonical-json",target_field:"candidate-visibility-field",original_value:{kind:"code",value:"VISIBLE"},replacement_value:{kind:"code",value:"TOMBSTONED"}}`；
`hard-delete={mutation_id:"hard-delete",encoding:"canonical-json",target_field:"candidate-visibility-field",original_value:{kind:"code",value:"VISIBLE"},replacement_value:{kind:"code",value:"HARD_DELETED"}}`；
`partial-build={mutation_id:"partial-build",encoding:"canonical-json",target_field:"candidate-build-field",original_value:{kind:"code",value:"BUILD_COMPLETE"},replacement_value:{kind:"code",value:"BUILD_PARTIAL"}}`；
`corrupt-manifest={mutation_id:"corrupt-manifest",encoding:"canonical-json",target_field:"manifest-field",original_value:{kind:"code",value:"MANIFEST_WELL_FORMED"},replacement_value:{kind:"code",value:"MANIFEST_CORRUPT"}}`；
`corrupt-index={mutation_id:"corrupt-index",encoding:"canonical-json",target_field:"index-field",original_value:{kind:"code",value:"INDEX_WELL_FORMED"},replacement_value:{kind:"code",value:"INDEX_CORRUPT"}}`；
`interrupted-build={mutation_id:"interrupted-build",encoding:"canonical-json",target_field:"build-worker-field",original_value:{kind:"code",value:"WORKER_COMPLETES"},replacement_value:{kind:"code",value:"WORKER_INTERRUPTED"}}`；
`interrupted-publish={mutation_id:"interrupted-publish",encoding:"canonical-json",target_field:"publish-worker-field",original_value:{kind:"code",value:"WORKER_COMPLETES"},replacement_value:{kind:"code",value:"WORKER_INTERRUPTED"}}`；
`interrupted-cutover={mutation_id:"interrupted-cutover",encoding:"canonical-json",target_field:"cutover-worker-field",original_value:{kind:"code",value:"WORKER_COMPLETES"},replacement_value:{kind:"code",value:"WORKER_INTERRUPTED"}}`；
`missing-dependency={mutation_id:"missing-dependency",encoding:"canonical-json",target_field:"dependency-acquisition-field",original_value:{kind:"code",value:"DEPENDENCY_PRESENT"},replacement_value:{kind:"code",value:"DEPENDENCY_ABSENT"}}`；
`outbound-network-or-service-attempt={mutation_id:"outbound-network-or-service-attempt",encoding:"canonical-json",target_field:"network-egress-field",original_value:{kind:"code",value:"EGRESS_DENIED"},replacement_value:{kind:"code",value:"EGRESS_ATTEMPTED"}}`。
No registry row may satisfy `original_value == replacement_value` under the typed-object comparison; a code variant and an
identity variant are never equal even when their character lengths coincide, and the two owner/source rows use distinct
identity literals. `canonical mutation bytes` means the exact canonical UTF-8 bytes of that complete `MUTATION_SPEC` object,
serialized with `sa-json-c14n-v1`; no implementation-defined encoding, offset, or prose value is allowed.
`mutation_sha256=SHA256(LP(mutation_preimage_domain)||LP(fixture_id)||LP(workload)||LP(decimal(instance_ordinal))||LP(canonical mutation bytes))`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `unauthorized-owner` | `input-owner-metadata` | 1K:3×20 | query | true | `BACKEND_PREFLIGHT_OWNER_DENIED/BACKEND_RUNTIME_OWNER_DENIED` | false | false |
| `cross-source` | `input-source-metadata` | 1K:3×20 | query | true | `BACKEND_PREFLIGHT_SOURCE_DENIED/BACKEND_RUNTIME_SOURCE_DENIED` | false | false |
| `stale-generation` | `input-generation-metadata` | 1K:3×20 | query | true | `BACKEND_PREFLIGHT_STALE_GENERATION/BACKEND_RUNTIME_STALE_GENERATION` | false | false |
| `stale-snapshot` | `input-snapshot-metadata` | 1K:3×20 | query | true | `BACKEND_PREFLIGHT_STALE_SNAPSHOT/BACKEND_RUNTIME_STALE_SNAPSHOT` | false | false |
| `wrong-dimension` | `input-vector-dimension` | 1K:3×20 | metadata | false | `BACKEND_PREFLIGHT_DIMENSION_MISMATCH/BACKEND_RUNTIME_DIMENSION_MISMATCH` | false | false |
| `wrong-vector-profile` | `input-vector-profile` | 1K:3×20 | metadata | false | `BACKEND_PREFLIGHT_VECTOR_PROFILE_MISMATCH/BACKEND_RUNTIME_VECTOR_PROFILE_MISMATCH` | false | false |
| `tombstone` | `candidate-tombstone` | 1K:3×20; 100K:1×10 | query | true | `BACKEND_PREFLIGHT_TOMBSTONE_HIDDEN/BACKEND_RUNTIME_TOMBSTONE_HIDDEN` | true | false |
| `hard-delete` | `candidate-hard-delete` | 1K:3×20; 100K:1×10 | query | true | `BACKEND_PREFLIGHT_DELETE_HIDDEN/BACKEND_RUNTIME_DELETE_HIDDEN/NTSTATUS_CLEANUP_OBJECT_NOT_FOUND` | true | false |
| `partial-build` | `candidate-partial-build` | 1K:3×20; 100K:1×10 | metadata | true | `BACKEND_PREFLIGHT_PARTIAL_BUILD_REJECTED/BACKEND_RUNTIME_PARTIAL_BUILD_REJECTED/BACKEND_CLEANUP_PARTIAL_BUILD_REJECTED` | true | true |
| `corrupt-manifest` | `manifest-read` | 1K:3×20; 100K:1×10 | metadata | true | `BACKEND_PREFLIGHT_MANIFEST_CORRUPT/BACKEND_RUNTIME_MANIFEST_CORRUPT/BACKEND_CLEANUP_ROLLBACK_VERIFIED` | true | true |
| `corrupt-index` | `index-read` | 1K:3×20; 100K:1×10 | metadata | true | `BACKEND_PREFLIGHT_INDEX_CORRUPT/BACKEND_RUNTIME_INDEX_CORRUPT/BACKEND_CLEANUP_ROLLBACK_VERIFIED` | true | true |
| `interrupted-build` | `build-worker` | 1K:3×20; 100K:1×10 | launcher | true | `BACKEND_PREFLIGHT_BUILD_INTERRUPTED/BACKEND_RUNTIME_BUILD_INTERRUPTED/BACKEND_CLEANUP_ROLLBACK_VERIFIED` | true | true |
| `interrupted-publish` | `publish-worker` | 1K:3×20; 100K:1×10 | launcher | true | `BACKEND_PREFLIGHT_PUBLISH_INTERRUPTED/BACKEND_RUNTIME_PUBLISH_INTERRUPTED/BACKEND_CLEANUP_ROLLBACK_VERIFIED` | true | true |
| `interrupted-cutover` | `cutover-worker` | 1K:3×20; 100K:1×10 | launcher | true | `BACKEND_PREFLIGHT_CUTOVER_INTERRUPTED/BACKEND_RUNTIME_CUTOVER_INTERRUPTED/BACKEND_CLEANUP_ROLLBACK_VERIFIED` | true | true |
| `missing-dependency` | `dependency-acquisition` | 1K:3×20 | launcher | false | `BACKEND_PREFLIGHT_DEPENDENCY_MISSING/BACKEND_RUNTIME_DEPENDENCY_MISSING` | false | false |
| `outbound-network-or-service-attempt` | `network-egress` | 1K:3×20 | network | false | `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED/WATCHDOG_RUNTIME_OUTBOUND_BLOCKED` | false | false |

Table shorthands map exactly: 1K=`1k-correctness`, 100K=`100k-capacity`; query/metadata/launcher/network append `-assertion`.
Every row's complete `mutation.mutation_id` equals `fixture_id`, and its five-field `MUTATION_SPEC` is repeated unchanged in every expanded fixture receipt; `mutation_preimage_domain` is `sa-m8-fault-<fixture-id>-v1`; `precondition` is
...
`{entries:A<STATUS_MAP_ENTRY;72..72;order=key(source_domain,phase,raw_status);unique=key(source_domain,phase,raw_status)>,
required_stable_codes:A<STABLE_CODE;72..72;order=value;unique=value>}`。
`STATUS_MAP_ENTRY` exact fields：`{source_domain:enum[backend,ntstatus,protocol,watchdog],phase:enum[preflight,runtime,cleanup],
raw_status:CODE,stable_code:STABLE_CODE}`。`source_domain` 的 schema 声明顺序就是以下表的冻结 domain 顺序
`backend→ntstatus→protocol→watchdog`；按 §2.3 的 enum 比较器使用该声明顺序，不存在 `win32` 或其他隐含 domain。
`entries` 是以下 72 行的精确有序集合，不是名称集合；序列化时按
`key(source_domain,phase,raw_status)` 排序。stable code 是其 source-domain、phase 和 raw-status 的唯一合成名称；
同一 raw status 在不同 phase 具有不同 stable code，禁止从 raw status 推断 phase。

| source_domain | phase | raw_status | stable_code |
| --- | --- | --- | --- |
| `backend` | `cleanup` | `BUILD_INTERRUPTED` | `BACKEND_CLEANUP_BUILD_INTERRUPTED` |
| `backend` | `cleanup` | `CUTOVER_INTERRUPTED` | `BACKEND_CLEANUP_CUTOVER_INTERRUPTED` |
| `backend` | `cleanup` | `INDEX_CORRUPT` | `BACKEND_CLEANUP_INDEX_CORRUPT` |
| `backend` | `cleanup` | `MANIFEST_CORRUPT` | `BACKEND_CLEANUP_MANIFEST_CORRUPT` |
| `backend` | `cleanup` | `PARTIAL_BUILD_REJECTED` | `BACKEND_CLEANUP_PARTIAL_BUILD_REJECTED` |
| `backend` | `cleanup` | `PUBLISH_INTERRUPTED` | `BACKEND_CLEANUP_PUBLISH_INTERRUPTED` |
| `backend` | `cleanup` | `ROLLBACK_VERIFIED` | `BACKEND_CLEANUP_ROLLBACK_VERIFIED` |
| `backend` | `preflight` | `BUILD_INTERRUPTED` | `BACKEND_PREFLIGHT_BUILD_INTERRUPTED` |
| `backend` | `preflight` | `CUTOVER_INTERRUPTED` | `BACKEND_PREFLIGHT_CUTOVER_INTERRUPTED` |
| `backend` | `preflight` | `DELETE_HIDDEN` | `BACKEND_PREFLIGHT_DELETE_HIDDEN` |
| `backend` | `preflight` | `DEPENDENCY_MISSING` | `BACKEND_PREFLIGHT_DEPENDENCY_MISSING` |
| `backend` | `preflight` | `DIMENSION_MISMATCH` | `BACKEND_PREFLIGHT_DIMENSION_MISMATCH` |
| `backend` | `preflight` | `INDEX_CORRUPT` | `BACKEND_PREFLIGHT_INDEX_CORRUPT` |
| `backend` | `preflight` | `MANIFEST_CORRUPT` | `BACKEND_PREFLIGHT_MANIFEST_CORRUPT` |
| `backend` | `preflight` | `OWNER_DENIED` | `BACKEND_PREFLIGHT_OWNER_DENIED` |
| `backend` | `preflight` | `PARTIAL_BUILD_REJECTED` | `BACKEND_PREFLIGHT_PARTIAL_BUILD_REJECTED` |
| `backend` | `preflight` | `PUBLISH_INTERRUPTED` | `BACKEND_PREFLIGHT_PUBLISH_INTERRUPTED` |
| `backend` | `preflight` | `SOURCE_DENIED` | `BACKEND_PREFLIGHT_SOURCE_DENIED` |
| `backend` | `preflight` | `STALE_GENERATION` | `BACKEND_PREFLIGHT_STALE_GENERATION` |
| `backend` | `preflight` | `STALE_SNAPSHOT` | `BACKEND_PREFLIGHT_STALE_SNAPSHOT` |
| `backend` | `preflight` | `TOMBSTONE_HIDDEN` | `BACKEND_PREFLIGHT_TOMBSTONE_HIDDEN` |
| `backend` | `preflight` | `VECTOR_PROFILE_MISMATCH` | `BACKEND_PREFLIGHT_VECTOR_PROFILE_MISMATCH` |
| `backend` | `runtime` | `BUILD_INTERRUPTED` | `BACKEND_RUNTIME_BUILD_INTERRUPTED` |
| `backend` | `runtime` | `CUTOVER_INTERRUPTED` | `BACKEND_RUNTIME_CUTOVER_INTERRUPTED` |
| `backend` | `runtime` | `DELETE_HIDDEN` | `BACKEND_RUNTIME_DELETE_HIDDEN` |
| `backend` | `runtime` | `DEPENDENCY_MISSING` | `BACKEND_RUNTIME_DEPENDENCY_MISSING` |
| `backend` | `runtime` | `DIMENSION_MISMATCH` | `BACKEND_RUNTIME_DIMENSION_MISMATCH` |
| `backend` | `runtime` | `INDEX_CORRUPT` | `BACKEND_RUNTIME_INDEX_CORRUPT` |
| `backend` | `runtime` | `MANIFEST_CORRUPT` | `BACKEND_RUNTIME_MANIFEST_CORRUPT` |
| `backend` | `runtime` | `OWNER_DENIED` | `BACKEND_RUNTIME_OWNER_DENIED` |
| `backend` | `runtime` | `PARTIAL_BUILD_REJECTED` | `BACKEND_RUNTIME_PARTIAL_BUILD_REJECTED` |
| `backend` | `runtime` | `PUBLISH_INTERRUPTED` | `BACKEND_RUNTIME_PUBLISH_INTERRUPTED` |
| `backend` | `runtime` | `ROLLBACK_VERIFIED` | `BACKEND_RUNTIME_ROLLBACK_VERIFIED` |
| `backend` | `runtime` | `SOURCE_DENIED` | `BACKEND_RUNTIME_SOURCE_DENIED` |
| `backend` | `runtime` | `STALE_GENERATION` | `BACKEND_RUNTIME_STALE_GENERATION` |
| `backend` | `runtime` | `STALE_SNAPSHOT` | `BACKEND_RUNTIME_STALE_SNAPSHOT` |
| `backend` | `runtime` | `TOMBSTONE_HIDDEN` | `BACKEND_RUNTIME_TOMBSTONE_HIDDEN` |
| `backend` | `runtime` | `VECTOR_PROFILE_MISMATCH` | `BACKEND_RUNTIME_VECTOR_PROFILE_MISMATCH` |
| `ntstatus` | `cleanup` | `FLUSH_FAILED` | `NTSTATUS_CLEANUP_FLUSH_FAILED` |
| `ntstatus` | `cleanup` | `OBJECT_NOT_FOUND` | `NTSTATUS_CLEANUP_OBJECT_NOT_FOUND` |
| `ntstatus` | `preflight` | `ACCESS_DENIED` | `NTSTATUS_PREFLIGHT_ACCESS_DENIED` |
| `ntstatus` | `preflight` | `OBJECT_NAME_COLLISION` | `NTSTATUS_PREFLIGHT_OBJECT_NAME_COLLISION` |
| `ntstatus` | `preflight` | `OBJECT_NOT_FOUND` | `NTSTATUS_PREFLIGHT_OBJECT_NOT_FOUND` |
| `ntstatus` | `preflight` | `REPARSE_REJECTED` | `NTSTATUS_PREFLIGHT_REPARSE_REJECTED` |
| `ntstatus` | `preflight` | `SHARING_VIOLATION` | `NTSTATUS_PREFLIGHT_SHARING_VIOLATION` |
| `protocol` | `cleanup` | `PACKAGE_TIMEOUT` | `PROTOCOL_CLEANUP_PACKAGE_TIMEOUT` |
| `protocol` | `cleanup` | `READBACK_MISMATCH` | `PROTOCOL_CLEANUP_READBACK_MISMATCH` |
| `protocol` | `preflight` | `CONTAINMENT_UNSUPPORTED` | `PROTOCOL_PREFLIGHT_CONTAINMENT_UNSUPPORTED` |
| `protocol` | `runtime` | `INVALID_PROTOCOL_DEVIATION` | `PROTOCOL_RUNTIME_INVALID_PROTOCOL_DEVIATION` |
| `protocol` | `runtime` | `NOT_RUN_GATE_STOP` | `PROTOCOL_RUNTIME_NOT_RUN_GATE_STOP` |
| `watchdog` | `cleanup` | `CLEANUP_TIMEOUT` | `WATCHDOG_CLEANUP_CLEANUP_TIMEOUT` |
| `watchdog` | `preflight` | `ACQUISITION_TIMEOUT` | `WATCHDOG_PREFLIGHT_ACQUISITION_TIMEOUT` |
| `watchdog` | `preflight` | `DEPENDENCY_VERIFY_TIMEOUT` | `WATCHDOG_PREFLIGHT_DEPENDENCY_VERIFY_TIMEOUT` |
| `watchdog` | `preflight` | `GOLD_GENERATION_TIMEOUT` | `WATCHDOG_PREFLIGHT_GOLD_GENERATION_TIMEOUT` |
| `watchdog` | `preflight` | `INPUT_GENERATION_TIMEOUT` | `WATCHDOG_PREFLIGHT_INPUT_GENERATION_TIMEOUT` |
| `watchdog` | `preflight` | `LAUNCHER_HEALTH_TIMEOUT` | `WATCHDOG_PREFLIGHT_LAUNCHER_HEALTH_TIMEOUT` |
| `watchdog` | `preflight` | `OUTBOUND_BLOCKED` | `WATCHDOG_PREFLIGHT_OUTBOUND_BLOCKED` |
| `watchdog` | `preflight` | `PREFLIGHT_TIMEOUT` | `WATCHDOG_PREFLIGHT_PREFLIGHT_TIMEOUT` |
| `watchdog` | `runtime` | `CHILD_LEAK` | `WATCHDOG_RUNTIME_CHILD_LEAK` |
| `watchdog` | `runtime` | `CHILD_START_TIMEOUT` | `WATCHDOG_RUNTIME_CHILD_START_TIMEOUT` |
| `watchdog` | `runtime` | `DISK_FULL` | `WATCHDOG_RUNTIME_DISK_FULL` |
| `watchdog` | `runtime` | `FAULT_TIMEOUT` | `WATCHDOG_RUNTIME_FAULT_TIMEOUT` |
| `watchdog` | `runtime` | `LIFECYCLE_TIMEOUT` | `WATCHDOG_RUNTIME_LIFECYCLE_TIMEOUT` |
| `watchdog` | `runtime` | `OUTBOUND_BLOCKED` | `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED` |
| `watchdog` | `runtime` | `PROVISION_TIMEOUT` | `WATCHDOG_RUNTIME_PROVISION_TIMEOUT` |
| `watchdog` | `runtime` | `QUERY_REPETITION_TIMEOUT` | `WATCHDOG_RUNTIME_QUERY_REPETITION_TIMEOUT` |
| `watchdog` | `runtime` | `QUERY_TIMEOUT` | `WATCHDOG_RUNTIME_QUERY_TIMEOUT` |
| `watchdog` | `runtime` | `REOPEN_TIMEOUT` | `WATCHDOG_RUNTIME_REOPEN_TIMEOUT` |
| `watchdog` | `runtime` | `REPORT_TIMEOUT` | `WATCHDOG_RUNTIME_REPORT_TIMEOUT` |
| `watchdog` | `runtime` | `TOTAL_DEADLINE` | `WATCHDOG_RUNTIME_TOTAL_DEADLINE` |
| `watchdog` | `runtime` | `VERIFICATION_TIMEOUT` | `WATCHDOG_RUNTIME_VERIFICATION_TIMEOUT` |
| `watchdog` | `runtime` | `WATCHDOG_GAP` | `WATCHDOG_RUNTIME_WATCHDOG_GAP` |

The exact 72 stable-code values are the right-hand column above, sorted by `value`; `required_stable_codes` is exactly that
set and every stable code appears once. The fault registry uses the same qualified values for every phase-keyed expected code.
All protocol-generated stops must use their declared qualified row; unknown raw status, duplicate tuple, absent required code,
ambiguous phase, or undeclared alias is `INVALID_PROTOCOL_DEVIATION`. Raw backend errors never enter report/package.
...
| PACKAGE_WRITTEN | cleanup-writer/P7A authorization | package digest/readback exact、receipt target absent、cleanup writer 的可执行 source digest 等于 `cleanup_writer_source_sha256` | close descendants、handle-relative cleanup | CLEANUP_IN_PROGRESS | CLEANUP_FAILURE_RECORD（`branch=normal`）→CLEANUP_INCOMPLETE |
| CLEANUP_IN_PROGRESS | cleanup-writer/P7A authorization | ordered cleanup attempts complete、zero residual proven | normal receipt CREATE_NEW→single write→flush→close→reopen/readback | CLEANUP_VERIFIED | CLEANUP_FAILURE_RECORD（`branch=normal`）→CLEANUP_INCOMPLETE |
| NONPUBLICATION_CLEANUP_IN_PROGRESS | cleanup-writer/P7A cleanup-only authorization | raw evidence verified、every created publication target deleted and reopen-not-found | nonpublication receipt CREATE_NEW→single write→flush→close→reopen/readback | VALID_EVIDENCE_NOT_PUBLISHED | CLEANUP_FAILURE_RECORD（`branch=nonpublication`）→CLEANUP_INCOMPLETE |
| ABORT_CLEANUP_IN_PROGRESS | coordinator/cleanup-writer：P6 前用 P4 standing authorization，P6 后用 P4+P6 | any root/descendant exists after preflight/runtime/invalid or P5/P7 rejection | close descendants、delete root、abort receipt CREATE_NEW→single write→flush→close→reopen/readback | abort receipt 的 `pending_terminal` | CLEANUP_FAILURE_RECORD（`branch=abort`）→CLEANUP_INCOMPLETE |
...
abort cleanup 成功后，落定的终态是 abort receipt `pending_terminal` 字段的逐字值，且该值必须属于 §2.2 冻结的闭合 `ABORT_TERMINAL` 集合，不得由 §9 较宽的 `TERMINAL_STATE` 集合代替或扩张。
...
| `disk-cap` | resource | `pred-disk-cap-v1` | `aggregate_cap_bytes:INT=17179869184;aggregate_root_peak_bytes:INT;max_sample_root_bytes:INT;sample_cap_bytes:INT=8589934592` | `aggregate_root_peak_bytes<=aggregate_cap_bytes AND max_sample_root_bytes<=sample_cap_bytes` | `WATCHDOG_RUNTIME_DISK_FULL` | `C(resource_watchdog)` |
| `network-zero` | hard | `pred-network-zero-v1` | `egress_bytes:INT;network_connections:INT` | `egress_bytes==0 AND network_connections==0` | `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED` | `C(network_inventory),C(interposition_ledger)` |
| `listener-zero` | hard | `pred-listener-zero-v1` | `listeners:INT` | `listeners==0` | `WATCHDOG_RUNTIME_OUTBOUND_BLOCKED` | `C(environment)` |
| `service-process-zero` | hard | `pred-service-process-zero-v1` | `candidate_service_processes:INT` | `candidate_service_processes==0` | `WATCHDOG_RUNTIME_CHILD_LEAK` | `C(environment)` |
...
| `residual-zero` | hard | `pred-residual-zero-v1` | `child_processes:INT;open_handles:INT;residual_bytes:INT;residual_entries:INT` | `child_processes==0 AND open_handles==0 AND residual_bytes==0 AND residual_entries==0` | `WATCHDOG_RUNTIME_CHILD_LEAK` | `C(resource_watchdog)` |
...
被审 bytes。P0 record 必须填写 `drafting_party_name`，且 reviewer `actor.name` 与其逐字不同；相等、缺失或仅以 `basis` 声明独立均 fail closed。
P0 输出只能是：
