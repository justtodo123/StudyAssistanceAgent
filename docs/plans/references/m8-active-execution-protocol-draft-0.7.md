...
> 文档版本：`draft-0.7`（仅为文档修订号，不是 experiment ID 或 executable protocol ID）。
> 起草日期：2026-09-12，修订日期：2026-09-13。本文为 draft-0.7 修订；本文所有摘要均由外部记录保存，本文不自哈希。
...
> 修订依据（二）：draft-0.6 的 P0 技术文字审查判定 `RETURNED_FOR_REVISION / P0_NOT_ACCEPTED`，draft-0.7 修正其
> 四项机械缺陷：`SYSTEM_RESERVED_STREAM` 类型归属与引用位置、volume/device root 的 stream 判定域缺失、
> `allowed_system_streams` 基数从自由区间改为派生常量、以及 interposition ADS 禁令与 stream 查询信息类的遗漏。
> draft-0.5/draft-0.6 保持历史，不得 binding 或授权。协议字节的任何变化都会改变 `protocol_blob_sha256`，
...
- `STREAM_SCOPE`：`enum[file,directory,volume-root]`，是一个三值闭合枚举，不是 `leaf_kind` 的别名。`file` 与 `directory`
  对应该次 open 的 `leaf_kind`；`volume-root` 对应该次 open 的 `authoritative_handle_source` 所指向的 volume/device root
  handle，它既不是 `file` leaf 也不是 `directory` leaf，因此拥有独立取值。任何 stream 判定都必须先按本次打开的
  `leaf_kind` 与 `FileStandardInformation.Directory` 选定唯一的 `STREAM_SCOPE`：`leaf_kind=file` 取 `file`；
  `leaf_kind=directory` 且该 handle 不是 volume/device root 时取 `directory`；该 handle 是 volume/device root 时取
  `volume-root`。三者互斥且穷尽，无法选定即为 `ABORTED_PREFLIGHT`。
...
`NumberOfLinks=1`。stream 规则先按 §2.2 的 `STREAM_SCOPE` 选定唯一 scope，再按该 scope 的 allowlist 行判定，并且是闭合的：
`scope=file` 必须恰有 unnamed stream `:$DATA`，且除该 unnamed stream 外只允许出现 allowlist 中 scope 为 `file` 的
`SYSTEM_RESERVED_STREAM`；`scope=directory` 与 `scope=volume-root` 允许 unnamed stream 不出现，且除 allowlist 中该
scope 的 `SYSTEM_RESERVED_STREAM` 外不得有任何 stream 记录。任何未列入 allowlist 的 named stream、任何缺失的
unnamed stream、任何 allowlist 外部观测到的 stream 记录、任何无法选定唯一 scope 的 handle，均使本次操作 fail closed
并映射为稳定 CODE；执行前为 `ABORTED_PREFLIGHT`，执行中为 `INVALID_PROTOCOL_DEVIATION`，cleanup 中为 `CLEANUP_INCOMPLETE`。
