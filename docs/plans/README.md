...
| `m9-goal-driven-planning-plan.md` | 阶段执行计划 | M9 目标驱动计划、mastery 与偏差；`ADMITTED / IN_PROGRESS`（范围 `m9-plan-lifecycle-v1`）；生成/采纳/进度/偏差重规划已实现，外部 AI、mastery 写入与评测 workload 未闭合 |
