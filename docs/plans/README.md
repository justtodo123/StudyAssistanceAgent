...
- M6b 与 M7 都以 M6a 退出证据为共同必要前置，彼此不互为前置；M7 已取得基础设施 scope admission 与
  独立生产开工授权，当前已形成 Source Registry、manifest/parser、normalized document、source-local FULL/INCREMENTAL sync 与已冻结的 delete/isolation 局部合同；M8 依赖真实 M7 exit，M9 依赖 M7/M8，M10 依赖 M7–M9
...
`ADMITTED / IN_PROGRESS`，生产开工已授权且 Source Registry、manifest/parser、normalized document、source-local FULL/INCREMENTAL sync
与 delete/isolation 局部合同已冻结；M8–M10 保持阻断。最终状态以 [`docs/PLAN.md`](../PLAN.md) 为准。
...
| `m7-source-lifecycle-plan.md` | 阶段执行计划 | 基础设施范围 `ADMITTED / IN_PROGRESS`；Source Registry、manifest/parser、normalized document、source-local FULL/INCREMENTAL sync 与 delete/isolation 局部合同已冻结，Network 不在 scope 内 |
