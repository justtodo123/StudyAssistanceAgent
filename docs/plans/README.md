...
`ADMITTED / IN_PROGRESS`，生产开工已授权且 Source Registry、manifest/parser、normalized document、source-local FULL/INCREMENTAL/delete/isolation 与 FTS5/offline fail-closed 局部合同已落地；M8–M10 保持阻断。最终状态以 [`docs/PLAN.md`](../PLAN.md) 为准。
...
| `m7-source-lifecycle-plan.md` | 阶段执行计划 | 基础设施范围 `ADMITTED / IN_PROGRESS`；Source Registry、manifest/parser、normalized document、source-local FULL/INCREMENTAL/delete/isolation 与 FTS5/offline fail-closed 局部合同已冻结，Network 不在 scope 内 |
