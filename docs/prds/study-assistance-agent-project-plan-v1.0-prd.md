...
- **进度风险**：把准备计划当成开工许可。缓解：M6a–M10 保持 `BLOCKED / NOT_STARTED`，以 `docs/PLAN.md` 为准。
...
- [x] M6a–M10 仅做设定澄清与准备计划，不写入生产模块
- **交付物**：现行 MVP + 准备计划
- **时间**：已完成 / 持续维护

### 阶段2：核心开发
**目标**：按准入顺序推进，不改现有里程碑切分；本阶段仅适用于已获 `ADMITTED` 的阶段，当前 M6a–M10 均未进入核心开发
- [ ] **未来验收** M6a：契约与兼容骨架，不改正式 API（`BLOCKED / NOT_STARTED`）
