...
- M6b 与 M7 彼此不互为前置；M6b 当前为 `ADMITTED / COMPLETE`，M7 仅基础设施范围为
  `ADMITTED / IN_PROGRESS`，生产开工已授权且 M7-1 Source Registry 已实施；Network 不在 M7 scope 内，M8/Milvus 继续阻断。
...
- [ ] **未来验收** M7 后续同步、删除传播、检索隔离与 benchmark 仍待完成；Network 晋升另行逐文档批准，M7-1 完成不等于 M7 exit
...
- [x] M6a 准入前仅做设定澄清与准备计划；M6b 已按独立批准实现默认关闭的只读 preview；M7 基础设施已获得独立生产开工授权，
  M7-1 Source Registry 已进入实施；M8–M10 保持阻断
- **交付物**：现行 MVP + 准备计划
- **时间**：已完成 / 持续维护
...
M7-1 Source Registry 已进入实施并完成当前窄增量；M7 后续同步、删除传播、检索隔离和 benchmark 仍未完成；
M8–M10 继续阻断。
...
- [x] **现行保护** M7-1：独立 Source Registry 已实施（`ADMITTED / IN_PROGRESS`）；不等于 M7 退出
- [ ] **未来验收** M7 后续增量：用户源同步、删除传播、检索隔离与千级 benchmark；
