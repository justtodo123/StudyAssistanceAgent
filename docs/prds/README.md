...
> PRD、阶段计划和 JSON 登记表都不能单独批准阶段；M6a 当前授权来自完整门禁与负责人批准记录。
...
- M6b 与 M7 彼此不互为前置；各自的 Source/退出证据和保护基线必须独立留证，专属决策与批准也必须分别闭合。当前均为 `BLOCKED / NOT_STARTED`。
...
*创建：2026-08-24 · 维护：新增/修订 PRD 时同步本表*