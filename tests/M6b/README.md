...
`ADMITTED / COMPLETE`。后续 M7 已在独立人工批准后成为 `ADMITTED / COMPLETE`；其 Source Registry、manifest/parser、normalized document、source-local FULL/INCREMENTAL/delete/isolation 与 FTS5/vector/offline fail-closed 合同独立实施。M7 完成不修改本目录的 M6b 行为、测试或 closeout 证据，也不把 M6b preview 扩展到用户源。
