...
| M10 写权限与副作用台账（步骤 1：拒绝路径；步骤 2：crash-point 恢复；步骤 3：通用异步 job；步骤 4：发布门禁；步骤 5：reconcile/人工介入；步骤 6：可选 Runner 接线） | `tests/M10/`（`m10`，115 项；**尚未纳入 CI**） | 见下行 | `tests/regression/` + `tests/M9/` | `platform/tests/` + `tests/M5c/` | 证明写权限**默认拒绝**（生产 allowlist 冻结为空 ⇒ 什么都注册不了）、写 allowlist 与只读 preview allowlist **不相交**、确认令牌绑定 `(job_id, tool_name, 参数摘要)` 且**同令牌配不同参数被拒**、撤销使未决授权失效、审计**追加式且只留摘要**（用例断言参数原文不出现）；台账在**独立 SQLite 文件**中、版本化 meta 不匹配即 fail closed、**`pending` 先于领域写入**、同键重放返回已记录 effect、同键不同参数判冲突、六态跃迁合法性、checkpoint 每 job 单调、**schema 无存放参数/结果原文的列**、使用台账后学习状态库逐字节未变；并以**动态枚举**证明领域写者仍恰好是 `learning_store.py`、runner 模块**不 import 领域仓储**。**不证明**：接受面在生产可达（两模块**未接入 `main.py`**，无路由/开关/worker）、crash 矩阵的 10 点（步骤 2 起）、越权的端到端隔离、任何性能或容量 |
