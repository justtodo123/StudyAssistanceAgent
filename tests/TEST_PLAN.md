...
> 起始日期：2026-08-17 · 更新：2026-08-18（M0-M2、M3a、M3b 已完成；M3c/M3d 待推进）
...
| 测试范围 | 收集数量 | 当前结果 | 说明 |
|----------|----------|----------|------|
| 根级 `tests/` | 90 项 | M3c 的 10 项条件跳过 | 阶段测试 + 回归套件 |
| `platform/tests/` | 40 项 | 40 passed | 原始平台冒烟/功能测试 |
| M3b 验收门禁 | 114 项 | 114 passed | M0_M2（18）+ M3a（22）+ M3b（13）+ regression（21）+ 平台测试（40） |

M3c 的 10 项测试会在 `knowledge/interview/` 建立后自动启用。受限 Windows 环境若默认临时目录不可写，可使用 `pytest --basetemp=.tmp-test`。
