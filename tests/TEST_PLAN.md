...
│
├── M9/                     # M9 目标驱动计划（生成 / 生命周期 / 偏差 / 只读复习历史、mastery、Source 摘要与先修关系投影 / 受限检索接缝 / 默认关闭的外部 AI 路径与 1K 冻结比较 / 唯一写权威证据）
│   ├── README.md           # 范围、排除项与运行命令
...
**2026-09-21 当前全量读数**（CPython 3.13.3，`pytest tests/ -q`，M9 唯一写权威证据增量后）：**1242 collected、
1238 passed、1 skipped、3 failed**。（同一日更早的读数 1234 项已被本条取代：mastery 增量 +39、Source 摘要增量 +42、
先修关系增量 +62、受限检索接缝增量 +32、计划身份往返保真增量 +26、复习历史活投影增量 +18、准入留痕字段覆盖增量 +1、
外部 AI 路径增量 +53（`test_plan_ai_adapter.py` 50 + `test_plan_ai_benchmark.py` 3）、唯一写权威证据增量 +8。）
...
`tests/M9`（324 项，其中 `m9_benchmark` 3 项由**独立步骤**运行）同样自 2026-09-21 起纳入该步骤；`tests/M7`（270 项）仍不在 CI 中，因其 3 项 TXT 用例
