...
│   ├── test_effect_ledger.py       # 独立文件 + 版本化 meta、pending 先于领域写入、幂等重放与冲突、六态、checkpoint
