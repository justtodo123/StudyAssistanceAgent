...
本目录覆盖现为 `ADMITTED / COMPLETE` 的 M7 基础设施范围：M7-1 Source Registry、单源 FULL 构建链、
source-local FULL/INCREMENTAL sync worker、已冻结的 source-local delete/isolation 合同、source-local FTS5/offline
fail-closed 校验与显式 FULL repair，以及 generation-bound source-local vector 与 FTS5 identity-set 合同。技术测试与
benchmark 本身不产生阶段批准；2026-09-06 的独立人工完成批准仅适用于 `m7-infrastructure-only-v1`，不授权 Network
或 M8–M10。
...
- sync/delete/sweep 共享 process-local operation `RLock`：两类合法并发顺序、锁外 barrier retry、嵌套重入、
  单 manifest/receipt/终态均由 Event/Barrier/stage hook 确定性覆盖；这不是跨进程 read lease；
...
- `sa.source.delete.v1` 的删除命令 CAS、`DELETE_PENDING` 读屏障、文件级 tombstone、
  ...
  故障恢复、30 天保留后 hard-delete receipt（含实际 FTS5/vector generation 目录清理），以及派生 provenance 的 `STALE_DERIVATION`；
...
- `use_vector=false` 仅执行授权 FTS5 route，不要求 vector runtime/metadata/query encoding；fresh/cache-hit 均在
...
- M7-6：真实运行时生成的 Markdown/TXT/PDF/PPTX/DOCX fixture parser、normalized-document 和 source lifecycle/provenance E2E 已加入；最近受影响 focused suite（M6b preview、source sync/offline、Search/vector）共 78 项通过，parser/normalized-document/lifecycle E2E 另行覆盖；离线索引在 generation 变化后仍需显式 FULL repair。
...
等二进制文件。M7-6 在 Python 3.11.9 和精确冻结依赖下生成合法五格式 fixture，验证公开 bytes/file parser、normalized-document
稳定性，以及 Source 注册→FULL/INCREMENTAL→Search/provenance→重启→删除屏障→hard-delete receipt 的 E2E。
`tools/run_m7_parser_evidence.py` 另以 seed `20260904` 完成每格式 100 fixtures × 20 runs 的冻结协议；
2026-09-06 在 `platform/.venv311` 的 CPython 3.11.9 精确依赖环境中独立复跑，五格式全 PASS，
parser/normalization/cold-restart/deterministic identity 失败均为 0，`external_source_reads=0`、`tmp_only=true`，报告仅写入系统临时目录且不入库。该 parser/normalization/identity 证据与独立 lifecycle/provenance E2E、
1k/3k BGE benchmark 分离，仍不自动构成整个 M7 exit。

FULL/INCREMENTAL、delete/isolation、FTS5/offline 与 generation-bound vector 仍是 source-local 工件；Search/QA 仅在服务端可信内部边界提供 `principal_id` 时叠加授权用户源。
