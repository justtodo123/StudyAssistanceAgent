...
class GoalPlannerService:
    ...
    def _distribute(
        ...
        变成 `daily_minutes` 的任意倍数，静默违反 `hours_per_day`。实测（2026-09-22，14 天窗口；
        分母是**该天的可用容量** `hours_per_day × 60 − 10` 复习缓冲）：默认请求 3890/110 = 35.4 倍、
        `1 小时/天` 4590/50 = 91.8 倍、`0.5 小时/天` 4590/20 = **229.5 倍**（探针集内最高）。
        倍数**依赖分母口径**，引用时须连分母一起引用；`8 小时/天` 下窗口内就排完，根本不触发。

        已知下界：`and day_tasks` 守卫保证每天的第一个任务必被放入，故单条任务的时长若超过当日
        容量，该天仍会超出。最小原子任务是 25 分钟，故**第 2 天起**（当日容量扣了复习缓冲，
        `hours_per_day × 60 − 10 < 25`，即 `hours_per_day < 35/60 ≈ 0.5833`）**每一天**都踩到
        这条守卫，容量保证在该区间内**完全空转**。**第 1 天不扣缓冲**（`build_day` 里仅当 `d > 0`
        才减 `REVIEW_BUFFER_MINUTES`），容量是 `hours_per_day × 60`，只有当日首条任务本身就超过它
        时才超出——要靠最小任务保证则需 `hours_per_day < 25/60 ≈ 0.4167`。`0.5 小时/天`（合法下界）
        下默认请求（全部课程）实测 142 天，其中 120 天超出声明的每日上界（`0.5 × 60 + 10 = 40`
        分钟），最大 60 分钟；第 1 天也在其中，因为该请求的首条任务是 50 分钟 > 30。残留的准确表述
        是「每天至多**一条**任务造成超出」，不是「绝不超出」。空任务集仍产出 1 个空天（保持既有行为）。
        """
