...
ALLOWED_EXECUTABLE = r"D:\Git\mingw64\bin\curl.exe"
ALLOWED_EXECUTABLE_SHA256 = "893476b0d2cb31a0efe1e22e0eb7698304db7add9c05efd99b66b20d7632d3b2"
ALLOWED_EXECUTABLE_SIZE = 313004
ALLOWED_EXECUTABLE_VERSION = "curl 8.12.1 (x86_64-w64-mingw32)"
ALLOWED_TLS_BACKEND = "Schannel"
PREPARATION_ROOT = PureWindowsPath(r"D:\面试实习\m8-network-acquisition-cycle-20260919-r02")
CURL_WRITE_OUT = "%{http_code}\\t%{size_download}\\t%{url_effective}"
CURL_FIXED_ARGUMENTS = (
    "--disable", "--silent", "--show-error", "--request", "GET", "--proto", "=https",
    "--connect-timeout", "15", "--max-time", "300", "--retry", "1",
    "--retry-max-time", "300", "--proxy", "",
)
...
def _host(url: str, *, initial: bool) -> str:
    ...
    _require(url.startswith("https://"), "M8ACQ_E001_INVALID_EVENT", "canonical scheme")
    ...
    _require(port is None, "M8ACQ_E001_INVALID_EVENT", "url port")
...
def observe_network(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(type(event["tls_verified"]) is bool, "M8ACQ_E001_INVALID_EVENT", "tls")
    _require(event["tls_verified"] is True, "M8ACQ_E006_TLS_UNVERIFIED", "tls")
    _require(type(event["certificate_bypass"]) is bool, "M8ACQ_E001_INVALID_EVENT", "certificate")
...
def observe_process(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(executable == ALLOWED_EXECUTABLE, "M8ACQ_E011_PROCESS_DENIED", "executable")
    ...
    _require(type(event["shell"]) is bool, "M8ACQ_E001_INVALID_EVENT", "shell")
...
def observe_write(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(type(event["is_reparse_point"]) is bool, "M8ACQ_E001_INVALID_EVENT", "reparse point")
...
def observe_redaction(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(type(content) is bytes, "M8ACQ_E001_INVALID_EVENT", "content_bytes")
...
def _strict_loads(data: bytes) -> Any:
    _require(type(data) is bytes, "M8ACQ_E001_INVALID_EVENT", "candidate bytes")
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {}
        for key, value in pairs:
            if key in result:
                raise ObservationError("M8ACQ_E001_INVALID_EVENT", "duplicate candidate key")
            result[key] = value
        return result
    try:
        value = json.loads(
            ...
            parse_constant=lambda value: (_ for _ in ()).throw(
                ObservationError("M8ACQ_E001_INVALID_EVENT", "non-finite candidate number")
            ),
        )
    except ObservationError:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError, ValueError, TypeError):
        raise ObservationError("M8ACQ_E001_INVALID_EVENT", "candidate json") from None
    _require(isinstance(value, dict), "M8ACQ_E001_INVALID_EVENT", "candidate object")
    return value


def _reject_surrogates(value: Any) -> None:
    if isinstance(value, str):
        _require(not any(0xD800 <= ord(character) <= 0xDFFF for character in value), "M8ACQ_E001_INVALID_EVENT", "candidate surrogate")
...
def _validate_candidate_shape(value: dict[str, Any], expected: Any, detail: str) -> None:
    if isinstance(expected, dict):
        _require(isinstance(value, dict), "M8ACQ_E001_INVALID_EVENT", detail)
        _require(set(value) == set(expected), "M8ACQ_E001_INVALID_EVENT", detail)
        for key, nested_expected in expected.items():
            _validate_candidate_shape(value[key], nested_expected, detail)
    elif expected is list:
        _require(isinstance(value, list), "M8ACQ_E001_INVALID_EVENT", detail)
    elif expected is bool:
        _require(type(value) is bool, "M8ACQ_E001_INVALID_EVENT", detail)
    elif expected is int:
        _require(type(value) is int, "M8ACQ_E001_INVALID_EVENT", detail)
    elif expected is str:
        _require(type(value) is str, "M8ACQ_E001_INVALID_EVENT", detail)


def validate_candidate_document(data: bytes) -> dict[str, Any]:
    candidate = _strict_loads(data)
    _reject_surrogates(candidate)
    expected_root_fields = {
        "acquisition_authorized", "allowed_actions", "allowed_next_action", "authority_scope",
        "backend_selection_authorized", "branch", "candidate_status", "canonicalization_id",
        "cycle_id", "cycle_type", "document_validation_policy", "downloader", "execution_boundary",
        "failure_policy", "forbidden_actions", "formal_identity_authorized", "formal_venv_authorized",
        "format", "git_binding", "installation_authorized", "limits", "m8_status",
        "network_access_authorized", "observer_binding", "owner_id", "owner_role", "parent_commit",
        "parent_history", "provenance_schema", "real_collector_authorized", "record_type",
        "repository", "required_reviews", "resolver_authorized", "runtime_authority", "s1_b_authorized",
        "s1_retry_authorized", "s2_authorized", "s3_authorized", "schema_version", "source_policy",
        "supersession", "wheel_download_authorized", "wheel_policy", "wheelhouse_creation_authorized",
        "write_boundary",
    }
    _require(set(candidate) == expected_root_fields, "M8ACQ_E001_INVALID_EVENT", "candidate root schema")
    _require(candidate.get("canonicalization_id") == CANONICALIZATION_ID, "M8ACQ_E001_INVALID_EVENT", "candidate canonicalization")
    _require(candidate.get("format") == "m8-network-acquisition-authority-candidate-v11", "M8ACQ_E001_INVALID_EVENT", "candidate format")
    _require(candidate.get("document_validation_policy", {}).get("schema_identifier") == "m8-network-acquisition-authority-candidate-v11", "M8ACQ_E001_INVALID_EVENT", "candidate schema")
    _require(candidate.get("downloader", {}).get("executable") == ALLOWED_EXECUTABLE, "M8ACQ_E011_PROCESS_DENIED", "candidate executable")
    _require(candidate.get("downloader", {}).get("working_directory") == str(PREPARATION_ROOT), "M8ACQ_E014_WRITE_OUTSIDE_ROOT", "candidate root")
    _require(candidate.get("source_policy", {}).get("allowed_initial_hosts") == ["pypi.org"], "M8ACQ_E003_INITIAL_HOST_DENIED", "candidate initial host")
    _require(candidate.get("source_policy", {}).get("allowed_schemes") == ["https"], "M8ACQ_E002_NON_HTTPS", "candidate scheme")
    _require(candidate.get("source_policy", {}).get("redirect_max_count") == 3, "M8ACQ_E005_REDIRECT_LIMIT", "candidate redirects")
    _require(candidate.get("document_validation_policy", {}).get("boolean_is_not_integer") is True, "M8ACQ_E001_INVALID_EVENT", "candidate bool policy")
    _require(candidate.get("document_validation_policy", {}).get("finite_numbers_only") is True, "M8ACQ_E001_INVALID_EVENT", "candidate finite policy")
    _require(candidate.get("document_validation_policy", {}).get("duplicate_object_keys") == "DENY", "M8ACQ_E001_INVALID_EVENT", "candidate duplicate policy")
    _require(candidate.get("document_validation_policy", {}).get("unknown_field_at_any_object_depth") == "FAIL_CLOSED", "M8ACQ_E001_INVALID_EVENT", "candidate unknown policy")
    return candidate


def validate_downloader_contract(candidate: Mapping[str, Any]) -> dict[str, Any]:
    downloader = _require_mapping(candidate.get("downloader"), "downloader")
    contract = _require_mapping(downloader.get("command_line_contract"), "command contract")
    _require(downloader.get("environment_allowlist") == [], "M8ACQ_E008_PROXY_PRESENT", "environment allowlist")
    environment = _require_mapping(downloader.get("environment_contract"), "environment contract")
    _require(environment.get("explicit_environment") == {}, "M8ACQ_E008_PROXY_PRESENT", "explicit environment")
    _require(environment.get("ambient_environment_inheritance") == "DENY", "M8ACQ_E008_PROXY_PRESENT", "ambient environment")
    _require(environment.get("proxy_environment_variables") == "DENY", "M8ACQ_E008_PROXY_PRESENT", "proxy environment")
    _require(contract.get("method") == "GET", "M8ACQ_E010_METHOD_DENIED", "command method")
    _require(contract.get("shell") is False, "M8ACQ_E013_SHELL_ENABLED", "command shell")
    _require(contract.get("config_disable_argument") == "--disable MUST_BE_FIRST_ARGUMENT", "M8ACQ_E001_INVALID_EVENT", "config argument")
    _require(contract.get("redirect_max_count") == 3, "M8ACQ_E005_REDIRECT_LIMIT", "command redirects")
    _require(contract.get("proxy") == "DENY_WITH_EMPTY_EXPLICIT_PROXY_AND_EMPTY_ENVIRONMENT", "M8ACQ_E008_PROXY_PRESENT", "command proxy")
    _require(contract.get("url_fragment_userinfo_sensitive_query") == "DENY", "M8ACQ_E009_AUTH_PRESENT", "command url policy")
    return {"observer": "candidate", "status": "PASS", "schema": candidate["format"]}
