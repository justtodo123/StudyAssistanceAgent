...
check("only P0 and P1 draft011 gates", sorted(path.name for path in (REFS / "external-gates").rglob("*draft011*.json")) == sorted([P0.name, P1.name]))
check("no draft011 P2", not list((REFS / "external-gates" / "p2").glob("*draft011*.json")))
