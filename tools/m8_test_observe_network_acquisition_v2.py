...
from tools.m8_observe_network_acquisition_v2 import (
    ERROR_CODES,
    ObservationError,
    observe_all,
    observe_network,
...
def fixtures() -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "write": {
            "path": r"D:\面试实习\m8-network-acquisition-cycle-20260919-r02\wheels\numpy.whl.partial",
...
def main() -> None:
    ...
    event["initial_url"] = "https://pypi.org:443/pypi/numpy/json"
    expect_code("M8ACQ_E001_INVALID_EVENT", lambda: observe_network(event))
    event = fixtures()["network"].copy()
    event["initial_url"] = "HTTPS://pypi.org/pypi/numpy/json"
    expect_code("M8ACQ_E001_INVALID_EVENT", lambda: observe_network(event))
    event = fixtures()["network"].copy()
    event["initial_url"] = "https://files.pythonhosted.org/packages/numpy.whl"
    ...
    for field in ("tls_verified", "certificate_bypass"):
        event = fixtures()["network"].copy()
        event[field] = 1
        expect_code("M8ACQ_E001_INVALID_EVENT", lambda event=event: observe_network(event))
    event = fixtures()["network"].copy()
    ...
    for value in (float("nan"), float("inf"), chr(0xD800)): 
        try:
            from tools.m8_observe_network_acquisition_v2 import canonical_bytes
    ...
    event["executable"] = r"C:\Program Files\Git\mingw64\bin\curl.exe"
    ...
    candidate_path = pathlib.Path(__file__).resolve().parents[1] / "docs" / "plans" / "references" / "external-artifacts" / "m8-network-acquisition-authority-candidate-20260919-r04.json"
    candidate_bytes = candidate_path.read_bytes()
    candidate = validate_candidate_document(candidate_bytes)
    validate_downloader_contract(candidate)
    bad_candidate = candidate_bytes.replace(b'"D:\\\\Git\\\\mingw64\\\\bin\\\\curl.exe"', b'"C:\\\\Program Files\\\\Git\\\\mingw64\\\\bin\\\\curl.exe"')
    expect_code("M8ACQ_E011_PROCESS_DENIED", lambda: validate_candidate_document(bad_candidate))
    expect_code("M8ACQ_E001_INVALID_EVENT", lambda: validate_candidate_document(candidate_bytes.replace(b'"format":"m8-network-acquisition-authority-candidate-v11"', b'"format":"m8-network-acquisition-authority-candidate-v11","unknown":true')))
    print("PASS: current-candidate schema and observer fail-closed controls")
