...
| 用户数据源 | 自定义知识目录，规模百→千→万 | 🔄 M7 Source Registry 到 FTS5/offline 局部合同已落地，Search/QA 可选 principal overlay 已接入；preview/quiz/sessions 仍不含用户源。1k/3k 仅有 disposable FTS5 证据且 vector 未挂接，不构成 M7 exit |
