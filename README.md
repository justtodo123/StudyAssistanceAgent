...
**当前状态**：M6a-P0 crawler 已收口；八项决策、保护基线与负责人批准已闭合；M6a-1 协议契约和 M6a-2 默认 `knowledge-pack` 兼容适配的自动化门禁已通过，M6a-3 尚未开始；M6a 保持
...
| Harness 框架 | M6a-1 provider-neutral 协议契约和 M6a-2 默认知识包兼容适配的自动化门禁已通过；M6a-3 确定性工具/状态机/静态额外源适配尚未开始。M6b–M10 仍被准入门禁阻断，只读预览与自主 Runner 尚未实现 | 🔄 M6a `ADMITTED / IN_PROGRESS` |
